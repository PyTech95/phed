    ...
    <Card className="clinic-card" data-testid="office-import-card">
      ...
      <CardContent className="space-y-3">
        <p className="text-sm text-slate-600">
          ...
          सिर्फ़ स्पष्ट PID या पहले से linked Consumer ID मान्य है। नाम/mobile से merge नहीं होगा। बिना ID या conflicting ID वाले rows अलग दिखेंगे। Valid records <b>Approved (green, locked)</b> होंगे।
        </p>
        ...
        {history.length > 0 && (
          <div className="text-xs text-slate-500 space-y-1" data-testid="office-history">
            {history.map((h) => (
              <div key={h.id} className="flex flex-wrap items-center justify-between gap-2 border-t pt-1" style={{ borderColor: 'var(--phed-border)' }} data-testid={`office-history-${h.id}`}>
                <span className="min-w-0 break-all">{h.filename} · {h.uploaded_by_name}</span>
                <span data-testid={`office-import-status-${h.id}`}>{h.status || 'Completed'} · ✓ {h.result?.approved} · ✗ {h.result?.not_found} · {new Date(h.created_at).toLocaleDateString('en-IN')}</span>
                <ImportDeleteButton kind="office" batch={h} onDeleted={() => { loadHistory(); if (result?.batch_id === h.id) setResult(null); }} />
              </div>
            ))}
          </div>
        )}
