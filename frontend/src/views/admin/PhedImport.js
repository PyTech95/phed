...
import { OfficeImportCard } from './OfficeImportCard';
...
export default function PhedImport() {
  ...
  const countCards = [
    ...
    ['Shared conn. numbers', c.shared_connection_numbers ?? 0],
  ];
                  ...
                  <div className="overflow-x-auto rounded-xl border" style={{ borderColor: 'var(--phed-border)' }}>
                    <Table>
                      <TableHeader><TableRow>
                        {['Row', 'Consumer ID', 'Property ID', 'Name', 'F/H', 'Phone', 'Water', 'Sewer', 'Type', 'Notes'].map((h) => <TableHead key={h}>{h}</TableHead>)}
                      ...
                      <TableBody>
                        {validation.preview.map((r) => (
                          <TableRow key={r.row}>
                            ...
                            <TableCell className="font-mono text-xs" data-testid={`import-preview-property-${r.row}`}>{r.property_id || '—'}</TableCell>
            ...
            <CardContent className="space-y-2 max-h-[420px] overflow-y-auto">
              ...
              {imports.map((im) => (
                <div key={im.id} className="rounded-lg border p-2.5 text-xs" style={{ borderColor: 'var(--phed-border)' }} data-testid={`import-history-${im.id}`}>
                  ...
                  <div style={{ color: 'var(--phed-muted)' }}>{im.counts?.total_rows} rows · {formatISTDateTime(im.created_at)}</div>
                  <ImportDeleteButton kind="consumer" batch={im} onDeleted={() => { loadImports(); if (validation?.id === im.id) setValidation(null); }} />
