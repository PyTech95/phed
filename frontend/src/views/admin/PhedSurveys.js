...
import { OfficeDocumentsStatus } from '../../components/OfficeDocumentsDeclaration';
...
import { printSurveySheet } from '../../lib/printSurvey';
import {
...
import {
  ...
  FileText, AlertTriangle, Droplet, Waves, Crosshair, ClipboardCheck, RotateCcw, Clock, Phone, Download, Search, Printer,
} from 'lucide-react';
...
export default function PhedSurveys() {
  ...
  useEffect(() => { load(); }, [load]);
  useLiveRefresh(load, !detailSurvey && !busy && !rowBusy && !bulkApproving);
  ...
  const openDetail = async (s) => {
    ...
    setDetailSurvey(s); setDetail(null); setRejectReason(''); setDetailLoading(true); setDetailError(false);
    try {
      const { data } = await axios.get(`${PHED}/property/${s.property_record_id}`, H());
      if (requestId === detailRequest.current) {
        setDetail(data);
        if (data.survey?.id === s.id) setDetailSurvey(data.survey);
      }
    } catch { if (requestId === detailRequest.current) setDetailError(true); }
    finally { if (requestId === detailRequest.current) setDetailLoading(false); }
  };
  const closeDetail = () => { ++detailRequest.current; setDetailSurvey(null); setDetail(null); setDetailLoading(false); };
  ...
  const printSurvey = () => {
    const ok = printSurveySheet({ survey: detailSurvey, property: detail?.property, consumers: detail?.consumers || [] });
    if (!ok) toast.error('Print window blocked — browser me popup allow karein');
  };
  ...
  return (
    <AdminLayout title="PHED Survey Review">
      <div className="mb-4 flex flex-wrap justify-between gap-3 text-sm">
        <span data-testid="phed-submission-source-label">PHED submissions</span>
        <Link className="text-blue-700 underline" to="/admin/notice-submissions" data-testid="legacy-notice-submissions-link">Notice submissions</Link>
      </div>
      <Card className="clinic-card mb-4">
        ...
        <div>
          ...
          <IndianDateInput
            value={f.date_from}
            onChange={(value) => { setPage(1); setF({ ...f, date_from: value }); }}
            label="From date" testId="survey-filter-date-from"
          />
        </div>
        <div>
          ...
          <IndianDateInput
            value={f.date_to}
            onChange={(value) => { setPage(1); setF({ ...f, date_to: value }); }}
            label="To date" testId="survey-filter-date-to"
          />
        </div>
        <Flt
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {rows?.length === 0 && <TableRow><TableCell colSpan={11} className="text-center py-10 text-slate-500">No surveys yet.</TableCell></TableRow>}
              {rows?.map((s) => (
                <TableRow key={s.id} data-testid={`survey-row-${s.id}`}>
                  ...
                  <TableCell className="min-w-[180px] max-w-[240px]" data-testid={`connection-${s.id}`}><ConnectionTypeBadge survey={s} testId={`connection-type-${s.id}`} /></TableCell>
                  <TableCell data-testid={`status-${s.id}`}><Badge className={`text-[10px] ${STATUS_STYLE[s.status] || ''}`}>{s.status}</Badge>
                    {s.water?.office_documents_submitted && <div className="mt-1 text-[10px] text-teal-700" data-testid={`office-documents-${s.id}`}>Documents at office</div>}
                  </TableCell>
                  <TableCell className="text-xs tabular-nums text-slate-600" data-testid={`submitted-${s.id}`}>{formatISTDateTime(s.submitted_at)}</TableCell>
                  <TableCell>
      ...
      <Dialog open={!!detailSurvey} onOpenChange={(o) => { if (!o) closeDetail(); }}>
        <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto" data-testid="survey-detail-dialog">
          ...
          {detailSurvey && (
            <div className="space-y-5 text-sm">
              <div className="flex flex-wrap items-center gap-2" data-testid="survey-review-header-summary">
                <Badge className={STATUS_STYLE[detailSurvey.status]}>{detailSurvey.status}</Badge>
                <ConnectionTypeBadge survey={detailSurvey} testId="survey-review-decision-badge" />
                ...
                <Button
                  ...
                  className="ml-auto h-9 border-blue-300 text-blue-700"
                  data-testid="print-survey-btn"
                >
              ...
              <div className="grid gap-4 lg:grid-cols-2" data-testid="survey-review-comparison">
                {detailLoading ? <div role="status" className="flex items-center gap-2 p-4 text-slate-500" data-testid="survey-property-loading"><Loader2 className="h-4 w-4 animate-spin" />Loading property details…</div> : <McPropertyDetails property={detail?.property} />}
                <PhedSurveyorDetails survey={detailSurvey} consumers={detail?.consumers || []} />
              </div>
              {detailError && <Button type="button" variant="outline" onClick={() => openDetail(detailSurvey)} data-testid="survey-property-retry"><RotateCcw className="mr-2 h-4 w-4" />Retry property details</Button>}

              <div className="rounded-lg border p-3" style={{ borderColor: 'var(--phed-border)' }} data-testid="survey-remarks-box">
              ...
              <div>
                ...
                <div className="space-y-1">
                  <OfficeDocumentsStatus water={detailSurvey.water} testId="review-office-documents-status" />
                  {(detailSurvey.attachments || []).length === 0 && <div className="text-xs text-slate-400" data-testid="review-no-photo-documents">{detailSurvey.water?.office_documents_submitted ? 'No digital copies attached' : 'No documents'}</div>}
              ...
              {['Submitted', 'Requires Review', 'Document Pending'].includes(detailSurvey.status) && (
                <Button onClick={approve} disabled={busy} className="h-14 w-full text-base font-semibold text-white shadow-sm" style={{ background: '#2E7D32' }} data-testid="approve-survey-btn">
                  {busy ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <CheckCircle2 className="w-5 h-5 mr-2" />} Approve survey
                </Button>
              )}
              ...
              {isAdmin && !['Draft', 'Rejected'].includes(detailSurvey.status) && (
                <div className="rounded-lg border p-3 space-y-2" style={{ borderColor: 'var(--phed-border)' }}>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button onClick={() => reopen('correction')} disabled={busy} variant="outline" className="h-12 text-sm font-medium text-orange-700 border-orange-300" data-testid="return-correction-btn">
                      {busy ? <Loader2 className="w-4 h-4 mr-1.5 animate-spin" /> : <RotateCcw className="w-4 h-4 mr-1.5" />} Return for correction
                    </Button>
                    <Button onClick={() => reopen('pending')} disabled={busy} variant="outline" className="h-12 text-sm font-medium text-orange-700 border-orange-300" data-testid="mark-pending-btn">
                      {busy ? <Loader2 className="w-4 h-4 mr-1.5 animate-spin" /> : <Clock className="w-4 h-4 mr-1.5" />} Mark pending
                    </Button>
                  </div>
                  <Button onClick={reject} disabled={busy} variant="outline" className="h-12 w-full text-sm font-semibold text-red-600 border-red-300" data-testid="reject-survey-btn">
          ...
          <DialogFooter><Button variant="outline" onClick={closeDetail} data-testid="survey-detail-close">Close</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </AdminLayout>
  );
...
function McPropertyDetails({ property }) {
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-center justify-between gap-3 border-b pb-2" style={{ borderColor: 'var(--phed-border)' }}>
        <h3 className="flex items-center gap-1.5 text-base font-bold" style={{ color: 'var(--phed-ink)' }}><MapPin className="h-4 w-4 text-blue-600" /> MC property data</h3>
...
function PhedSurveyorDetails({ survey, consumers }) {
  ...
  return (
    ...
    >
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b pb-2" style={{ borderColor: 'var(--phed-border)' }}>
        <h3 className="flex items-center gap-1.5 text-base font-bold" style={{ color: 'var(--phed-ink)' }}><Droplet className="h-4 w-4 text-teal-600" /> PHED &amp; surveyor data</h3>
      </div>
      <div className="mb-3 rounded-lg bg-slate-50 p-3" data-testid="phed-surveyor-highlight">
        <ConnectionTypeBadge survey={survey} testId="phed-surveyor-decision" />
        <div className="mt-2" data-testid="phed-surveyor-connection-availability">
          <ConnectionAvailability
            water={Boolean((water.connection_numbers || []).some(Boolean) || water.connection_number || water.has_water)}
            sewer={Boolean((water.sewer_connection_numbers || []).some(Boolean) || water.sewer_connection_number || water.has_sewer)}
            testId="review-connection-availability"
          />
        </div>
        <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
          <div><span className="text-slate-500">Colony</span><div className="font-semibold" style={{ color: 'var(--phed-ink)' }} data-testid="phed-surveyor-colony">{survey.colony_name || '—'}</div></div>
        </div>
      </div>
      <DetailPair label="Surveyor" value={survey.surveyor_name} testid="phed-surveyor-name" />
      <DetailPair label="Submitted date / time (IST)" value={formatISTDateTime(survey.submitted_at)} testid="phed-surveyor-submitted-at" />
      ...
      {water.office_document_status && (
        <DetailPair label="Office documents received" value={water.office_document_status} testid="phed-office-document-status" />
      )}
