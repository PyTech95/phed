...
import { phedState, phedMapColor } from '../../lib/phedMapStatus';
...
const createNumberedIcon = (number, status, id) => {
  // 3-colour rule: red = not surveyed, yellow = surveyor submitted (reopenable), green = admin approved
  const colors = {
    'Pending': '#ef4444',
    'Not Started': '#ef4444',
    'In Progress': '#eab308',
    ...
    'Completed': '#eab308',
    'Submitted': '#eab308',
    'Requires Review': '#eab308',
    'Document Pending': '#eab308',
    'Approved': '#22c55e',
    'No PHED Connection': '#22c55e',
    'Rejected': '#ef4444',
    'default': '#ef4444'
  };
  
  const color = phedMapColor(status);
  
  return L.divIcon({
    className: 'custom-numbered-marker',
    html: `<div data-testid="property-map-marker-${encodeURIComponent(id)}" data-status="${status}" style="
...
export default function PropertyMap() {
  ...
  const [moved, setMoved] = useState(null); // { id, latitude, longitude } pending save
  const [savingMove, setSavingMove] = useState(false);

  const onMarkerDragEnd = (property, e) => {
    const { lat, lng } = e.target.getLatLng();
    setMoved({ id: property.id, property_id: property.property_id, latitude: lat, longitude: lng });
  };
  ...
  const saveMove = async () => {
    ...
    try {
      ...
      setProperties((ps) => ps.map((p) => (p.id === moved.id ? { ...p, latitude: moved.latitude, longitude: moved.longitude } : p)));
      toast.success('Location save हो गई');
      setMoved(null);
    } catch (e) { toast.error(e.response?.data?.detail || 'Save failed'); } finally { setSavingMove(false); }
  };

  const cancelMove = () => { setMoved(null); setProperties((ps) => [...ps]); };
  ...
  useLiveRefresh(() => fetchProperties(), !placing && !moved && !showAdd);
  ...
  const fetchProperties = async () => {
    try {
      ...
      // Calculate stats
      const pending = props.filter(p => phedMapColor(phedState(p)) === '#ef4444').length;
      const completed = props.filter(p => phedMapColor(phedState(p)) === '#22c55e').length;
      const submitted = props.length - pending - completed;
      setStats({
        total: props.length,
        pending,
        completed,
    ...
    <div className="h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-[1000]">
        <div className="flex items-center justify-between px-4 h-14">
          <div className="flex items-center">
            <button
              data-testid="property-map-back" onClick={() => navigate('/employee')}
          ...
          <div className="flex items-center gap-2">
            <Button
              size="sm"
              variant="outline"
              data-testid="property-map-list" onClick={() => navigate('/employee/properties')}
            >
            ...
            <Button
              size="sm"
              data-testid="property-map-print" onClick={handlePrintMap}
            ...
            <Button
              ...
              disabled={loading}
              title="Refresh" data-testid="property-map-refresh"
            >
      ...
      <div className="bg-white border-b px-4 py-2">
        <div className="flex items-center justify-around text-center">
          <div>
            <p className="text-2xl font-bold text-slate-800" data-testid="property-map-total">{stats.total}</p>
            <p className="text-xs text-slate-500">Total</p>
          </div>
          <div className="h-8 w-px bg-slate-200" />
          <div>
            <p className="text-2xl font-bold text-red-500" data-testid="property-map-pending">{stats.pending}</p>
            <p className="text-xs text-slate-500">Pending</p>
          </div>
          <div className="h-8 w-px bg-slate-200" />
          <div>
            <p className="text-2xl font-bold text-emerald-600" data-testid="property-map-approved">{stats.completed}</p>
            <p className="text-xs text-slate-500">Approved</p>
          </div>
          <div><p className="text-2xl font-bold text-yellow-600" data-testid="property-map-submitted">{stats.submitted || 0}</p><p className="text-xs text-slate-500">In review / draft</p></div>
        </div>
          ...
          >
            ...
            {properties.filter(p => p.latitude && p.longitude).map((property, index) => (
              <Marker
                ...
                draggable={!placing}
                eventHandlers={{ dragend: (e) => onMarkerDragEnd(property, e) }}
                icon={createNumberedIcon(property.serial_number || index + 1, phedState(property), property.id)}
              >
                <Popup maxWidth={280}>
                  <div className="p-2 min-w-[200px]">
                    ...
                    <div className="flex gap-2 mt-2">
                      <div className="w-full text-[11px] mb-1" data-testid={`map-phed-status-${property.id}`}>
                        PHED: <span className="font-semibold">{phedState(property)}</span>
                      </div>
                    </div>
                    <div className="flex gap-2 mt-1">
                      ...
                      >
                        ...
                        {['Approved', 'No PHED Connection'].includes(property.phed_survey_status) ? 'View PHED Survey' : 'Water Bill Survey Update'}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
      ...
      {/* Pin moved: save or cancel */}
      {moved && !placing && (
        <div className="fixed left-4 right-4 bottom-24 z-[1200] bg-white rounded-2xl shadow-2xl p-3 flex items-center gap-2" data-testid="move-bar">
          <div className="flex-1 text-xs text-slate-700">
            <div className="font-semibold">{moved.property_id} pin हटाया गया</div>
            <div className="font-mono text-[10px] text-slate-500">{moved.latitude.toFixed(5)}, {moved.longitude.toFixed(5)}</div>
          </div>
          <Button variant="outline" className="h-11" onClick={cancelMove} data-testid="move-cancel-btn">Cancel</Button>
          <Button className="h-11 text-white bg-green-600 hover:bg-green-700" onClick={saveMove} disabled={savingMove} data-testid="move-save-btn">
            {savingMove ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Save location'}
          </Button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1100] bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => !adding && setShowAdd(false)} data-testid="add-property-modal">
          <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-4 space-y-3 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div><label className="text-xs font-medium text-slate-600">Colony *</label>
              <input className="w-full h-11 border rounded-lg px-3 mt-1" placeholder="Colony का नाम लिखें या चुनें" list="add-colony-options" value={addForm.colony} onChange={(e) => setAddForm({ ...addForm, colony: e.target.value })} data-testid="add-colony-input" />
              <datalist id="add-colony-options">
                {[...new Set(wards.flatMap((w) => w.colonies || []))].sort().map((c) => <option key={c} value={c} />)}
              </datalist>
            </div>
