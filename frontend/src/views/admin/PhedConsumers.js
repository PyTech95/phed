...
import { OfficeDocumentStatusFilter } from '../../components/OfficeDocumentStatusFilter';
...
export default function PhedConsumers() {
  ...
  const [f, setF] = useState({ search: '', ward_id: ANY, service: ANY, category: ANY, linked: ANY, source: ANY, colony: '', document_status: ALL_DOCUMENT_STATUSES });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (requestId !== loadRequestRef.current) return;
      toast.error(e.response?.data?.detail || 'Failed to load'); setRows([]); setTotal(0); setPages(0);
    }
  }, [page, f]);
    ...
    const t = setTimeout(async () => {
      try {
        const { data } = await axios.get(`${PHED}/consumers/search`, {
          ...
          params: { q, limit: 8, document_status: f.document_status === ANY ? undefined : f.document_status },
        });
        if (!cancelled) setSuggest(data.results || []);
      } catch { if (!cancelled) setSuggest([]); }
    }, 250);
    return () => { cancelled = true; controller.abort(); clearTimeout(t); };
  }, [f.search, f.document_status]);
      ...
      <Card className="clinic-card mb-4">
        <CardContent className="p-4">
          ...
          <div className="mt-3 max-w-md">
            <OfficeDocumentStatusFilter value={f.document_status} onChange={(value) => { setPage(1); setShowSuggest(false); setF({ ...f, document_status: value }); }} testId="admin-document-status-filter" />
          </div>
          <div className="flex flex-wrap items-center justify-between gap-3 mt-3">
            <span className="text-sm flex items-center gap-2" style={{ color: 'var(--phed-muted)' }}>
              <span data-testid="admin-filtered-consumer-count"><Users className="w-4 h-4 inline mr-1" />{rows === null ? 'Loading…' : `${total} consumers`}</span>
              {f.colony && (
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow>
                {['Consumer ID', 'Name', 'F/H Name', 'Phone', 'Locality', 'Connections', 'Category', 'Property', 'Survey', 'Document status', ''].map((h) => <TableHead key={h}>{h}</TableHead>)}
              ...
              <TableBody>
                ...
                {rows?.length === 0 && <TableRow><TableCell colSpan={11} className="text-center py-10 text-slate-500" data-testid="admin-consumers-empty-state">No records match the selected filters. Try another document status or clear your search.</TableCell></TableRow>}
                {rows?.map((c) => (
                  <TableRow key={c.id} data-testid={`consumer-row-${c.id}`}>
                    ...
                    <TableCell className="min-w-[200px] max-w-xs text-xs" data-testid={`admin-document-status-${c.id}`}>
                      {c.office_document_receipt?.status || 'Not submitted'}
                    </TableCell>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" data-testid="consumer-detail-dialog">
          ...
          {!detail ? <div className="py-10 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-600" /></div> : (
            <div className="space-y-5 text-sm">
              ...
              {detail.office_document_receipt && (
                <section className="rounded-lg border border-teal-200 bg-teal-50 p-3 space-y-1" data-testid="admin-office-document-receipt">
                  <p className="font-semibold">Office document receipt</p>
                  <p data-testid="admin-office-document-status">{detail.office_document_receipt.status}</p>
                  <p className="font-mono text-xs" data-testid="admin-office-document-property">Property ID: {detail.office_document_receipt.property_id}</p>
                  <p className="text-xs text-slate-600" data-testid="admin-office-document-received-by">{detail.office_document_receipt.received_by_name} · {formatISTDateTime(detail.office_document_receipt.received_at)}</p>
                  {detail.office_document_receipt.remarks && <p className="whitespace-pre-wrap text-xs" data-testid="admin-office-document-remarks">{detail.office_document_receipt.remarks}</p>}
                  <p className="text-xs text-slate-500">Receipt only — does not change the property link or survey status.</p>
                </section>
              )}
              <div>
