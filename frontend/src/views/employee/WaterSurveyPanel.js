...
import { AadhaarPhotoField } from '../../components/AadhaarPhotoField';
import { OfficeDocumentsCheckbox, OfficeDocumentsStatus } from '../../components/OfficeDocumentsDeclaration';
import { ConnectionTypeBadge, ConnectionAvailability } from '../../components/ConnectionStatus';
...
export default function WaterSurveyPanel({ property, onBack, onNext, H, initialConsumer = null }) {
  ...
  const [officeDocuments, setOfficeDocuments] = useState(false);
  ...
  useEffect(() => {
    ...
    axios.get(`${PHED}/property/${property.id}`, H()).then(({ data }) => {
      ...
      if (s && ['Document Pending', 'Draft', 'Submitted', 'Requires Review', 'Approved', 'Rejected'].includes(s.status)) {
        const w = s.water || {};
        setOfficeDocuments(w.office_documents_submitted === true);
        ...
        if (w.has_connection && (w.consumer_id || w.consumer_name)) {
          setPicked({ id: w.consumer_ref || s.consumer_ref || null, consumer_id: w.consumer_id || '', consumer_name: w.consumer_name || property.owner_name, category: w.category || 'Domestic', phone: w.phone || w.mobile || '' });
  ...
  const clearPicked = () => { setPicked(null); setWaterNos([]); setSewerNos([]); setOwnerChange(null); setOfficeDocuments(false); setSubmitError(''); };
  const changeOwner = (value) => { setOwnerChange(value); setOfficeDocuments(false); setSubmitError(''); };
  ...
  const isSatisfied = (type) => {
    if (type === 'AADHAAR') {
      return (aadhaar.front || existingDocTypes.has('AADHAAR_FRONT') || existingDocTypes.has('AADHAAR'))
        && (aadhaar.back || existingDocTypes.has('AADHAAR_BACK') || existingDocTypes.has('AADHAAR'));
    }
  ...
  const missingPhotoDocs = reqDocs.filter(([t, req]) => req && !isSatisfied(t)).map(([t]) => t);
  const officeDeclared = officeDocuments && reqDocs.length > 0;
  const missingCompulsory = officeDeclared ? [] : missingPhotoDocs;
  ...
  const stepBack = useCallback(() => {
    ...
    if (ownerChange) { changeOwner(null); return; }
    if (mode) { setMode(null); setDeniedReason(null); setOfficeDocuments(false); return; }
    onBack();
  ...
  const submit = async (officeOverride = officeDeclared) => {
    if (submittingRef.current || locked) return;
    const declareOffice = officeOverride === true && reqDocs.length > 0;
    const fail = (message) => { setSubmitError(message); toast.error(message); };
    setSubmitError('');
    if (!mode) return fail('बताइए — पानी का connection है, नहीं है, owner ने मना किया, या property locked है?');
    if (!effGps) return fail('Location नहीं मिली — "Retry GPS" दबाइए या map/property location चुनें');
    if (mode === 'yes' && !mobile.trim()) return fail('Mobile number ज़रूरी है');
    if (mode === 'yes' && !picked) return fail('PHED consumer चुनें');
    if (mode === 'no' && !nc.owner_name.trim()) return fail('Owner का नाम ज़रूरी है');
    if (mode === 'no' && !mobile.trim()) return fail('Owner का mobile number ज़रूरी है');
    if (mode === 'no' && !nc.locality.trim()) return fail('Colony / locality ज़रूरी है');
    if (mode === 'denied' && !deniedReason) return fail('मना करने का कारण चुनें');
    if (mode === 'denied' && deniedReason === 'OTHER' && !remarks.trim()) return fail('Other कारण लिखें');
    if (mode === 'yes' && ownerChange && !nc.owner_name.trim()) return fail('New owner का नाम ज़रूरी है');
    if (mode === 'yes' && ownerChange === 'DEATH_TRANSFER' && !nc.relationship.trim()) return fail('मृतक से रिश्ता ज़रूरी है');
    if (mode === 'yes' && ownerChange === 'OWNERSHIP_CHANGE' && !nc.change_reason.trim()) return fail('Ownership बदलने का कारण ज़रूरी है');
    if (!declareOffice && missingPhotoDocs.length > 0) return fail(`ज़रूरी document नहीं लगे: ${missingPhotoDocs.map((t) => DOC_LABELS[t] || t).join(', ')}`);
    ...
    try {
      ...
      const body = {
        ...
        remarks: remarks || null, consumer_refs: picked?.id ? [picked.id] : [], water,
        office_documents_submitted: declareOffice,
      };
      ...
      setSurvey({ ...s, submitted_at: r.submitted_at, connection_type: r.connection_type });
    ...
    } catch (e) { fail(typeof e.response?.data?.detail === 'string' ? e.response.data.detail : 'Submit failed. Please try again.'); } finally { submittingRef.current = false; setSubmitting(false); }
  };
  ...
  if (done) return (
    <EmployeeLayout title="Water Bill Survey">
      <div className="max-w-md mx-auto text-center py-12" data-testid="water-survey-done">
        ...
        <div className="mt-3"><ConnectionTypeBadge survey={survey} testId="survey-result-connection-type" /></div>
        <p className="mt-2 text-xs tabular-nums text-slate-600" data-testid="survey-result-submitted-at">{formatISTDateTime(survey?.submitted_at)}</p>
        <div className="mt-3"><OfficeDocumentsStatus water={survey?.water} testId="office-documents-submitted-confirmation" /></div>
  ...
  const AadhaarSlot = ([, req]) => {
    ...
    return (
      <div key="AADHAAR" className="rounded-lg" data-testid="doc-slot-AADHAAR">
        ...
        <div className="grid grid-cols-2 gap-2">
          {['front', 'back'].map((side) => <AadhaarPhotoField key={side} side={side} value={aadhaar[side]}
            onChange={(file) => setAadhaar((a) => ({ ...a, [side]: file }))} onPreview={openPreview}
            alreadyUploaded={side === 'front' ? haveFrontOld : haveBackOld} disabled={locked || submitting} />)}
        </div>
      </div>
    );
  };
  ...
  const documentSection = reqDocs.length > 0 && (
    <div className="space-y-3" data-testid="doc-section">
      <OfficeDocumentsCheckbox checked={officeDeclared} disabled={locked || submitting}
        onCheckedChange={(checked) => { setOfficeDocuments(checked); setSubmitError(''); if (checked) submit(true); }} />
      {!officeDeclared && reqDocs.map(renderDoc)}
    </div>
  );
      ...
      <div className="max-w-md mx-auto space-y-3 pb-44" data-testid="water-survey-panel">
        ...
        <Card className="clinic-card"><CardContent className="p-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <button type="button" disabled={locked || submitting} onClick={() => { if (mode !== 'yes') setOfficeDocuments(false); setMode('yes'); }} data-testid="water-yes-btn"
            ...
            <button type="button" disabled={locked || submitting} onClick={() => { setMode('denied'); clearPicked(); }} data-testid="owner-denied-btn"
            ...
            <button type="button" disabled={locked || submitting} onClick={() => { setMode('no'); clearPicked(); }} data-testid="water-no-btn"
          ...
          <button type="button" disabled={locked || submitting} onClick={() => { setMode('locked'); clearPicked(); setDeniedReason(null); }} data-testid="property-locked-btn"
          ...
          <Card className="clinic-card"><CardContent className="p-4 space-y-3" data-testid="water-yes-details">
                  ...
                  <div className="mt-2 space-y-1.5 max-h-64 overflow-y-auto" data-testid="consumer-search-results">
                    {results.map((c) => (
                      <button key={c.id} type="button" onClick={() => pickConsumer(c)} data-testid={`consumer-result-${c.id}`}
                        ...
                        <div className="mt-2"><ConnectionAvailability water={connsOf(c, 'Water').length > 0} sewer={connsOf(c, 'Sewer').length > 0} testId={`consumer-services-${c.id}`} /></div>
            ...
            {picked && (
              <div className="space-y-3" data-testid="picked-consumer">
                <ConnectionTypeBadge survey={{ water: { has_connection: true, connection_numbers: waterNos, sewer_connection_numbers: sewerNos, owner_change: ownerChange } }} testId="selected-consumer-connection-type" />
                <ConnectionAvailability water={waterNos.length > 0} sewer={sewerNos.length > 0} testId="selected-consumer-services" />
                ...
                <div className="rounded-lg border p-3" style={{ borderColor: 'var(--phed-border)' }}>
                  ...
                  <Row label="Billing category" value={<span data-testid="picked-category">{picked.category || 'Domestic'}</span>} />
                ...
                {!locked && (
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <button type="button" disabled={submitting} onClick={() => changeOwner(null)} data-testid="owner-same-btn" className={`h-10 rounded-lg border text-xs font-medium ${ownerChange === null ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600'}`}>Same owner</button>
                      <button type="button" disabled={submitting} onClick={() => changeOwner('DEATH_TRANSFER')} data-testid="owner-death-btn" className={`h-10 rounded-lg border text-xs font-medium ${ownerChange === 'DEATH_TRANSFER' ? 'border-purple-600 bg-purple-50 text-purple-700' : 'border-slate-200 text-slate-600'}`}>Death transfer</button>
                      <button type="button" disabled={submitting} onClick={() => changeOwner('OWNERSHIP_CHANGE')} data-testid="owner-change-btn" className={`h-10 rounded-lg border text-xs font-medium ${ownerChange === 'OWNERSHIP_CHANGE' ? 'border-purple-600 bg-purple-50 text-purple-700' : 'border-slate-200 text-slate-600'}`}>Ownership change</button>
                    </div>
        ...
        {mode === 'no' && (
          <Card className="clinic-card"><CardContent className="p-4 space-y-3" data-testid="new-connection-details">
            <ConnectionTypeBadge survey={{ survey_type: 'NO_CONNECTION' }} testId="new-connection-type" />
        ...
        <div className="fixed left-0 right-0 p-3 bg-white/95 backdrop-blur border-t z-40" style={{ borderColor: 'var(--phed-border)', bottom: 'calc(3.75rem + env(safe-area-inset-bottom))' }}>
          <div className="max-w-md mx-auto">
            {submitError && <p role="alert" className="mb-2 text-xs text-red-700" data-testid="water-survey-submit-error">{submitError}</p>}
