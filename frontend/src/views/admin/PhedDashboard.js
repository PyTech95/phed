...
export default function PhedDashboard() {
  ...
  const reportCards = [
    ...
    { label: 'New Connection', value: (s.by_outcome || {}).new_connection, icon: FilePlus, color: '#0F766E' },
    { label: 'Already Connection', value: (s.by_outcome || {}).has_connection, icon: Droplet, color: '#15803D' },
    { label: 'Water Connection Only', value: (s.by_outcome || {}).water_connection, icon: Droplet, color: '#1D4ED8' },
    { label: 'Sewer Connection Only', value: (s.by_outcome || {}).sewer_connection, icon: Waves, color: '#C2410C' },
    { label: 'Death Transfer', value: (s.by_outcome || {}).death_transfer, icon: FilePlus, color: '#BE123C' },
    { label: 'Ownership Change', value: (s.by_outcome || {}).ownership_change, icon: FilePlus, color: '#A16207' },
      ...
      <Card className="clinic-card mb-5">
        <CardContent className="p-4 grid md:grid-cols-4 gap-3">
          ...
          <div><div className="text-[11px] font-medium mb-1" style={{ color: 'var(--phed-muted)' }}>From</div>
            <IndianDateInput value={f.date_from} onChange={(v) => setF({ ...f, date_from: v })} testId="dash-filter-from" label="From date" /></div>
          <div><div className="text-[11px] font-medium mb-1" style={{ color: 'var(--phed-muted)' }}>To</div>
            <IndianDateInput value={f.date_to} onChange={(v) => setF({ ...f, date_to: v })} testId="dash-filter-to" label="To date" /></div>
        </CardContent>
      </Card>
