                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className={`w-full h-10 justify-start text-left font-normal ${!dateFilter ? 'text-muted-foreground' : ''}`}>
                      ...
                      {dateFilter ? format(new Date(dateFilter + 'T00:00:00'), 'dd/MM/yyyy') : 'DD/MM/YYYY'}
                    </Button>
                  </PopoverTrigger>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className={`w-full h-10 justify-start text-left font-normal ${!dateToFilter ? 'text-muted-foreground' : ''}`}>
                      <Calendar className="mr-2 h-4 w-4" />
                      {dateToFilter ? format(new Date(dateToFilter + 'T00:00:00'), 'dd/MM/yyyy') : 'DD/MM/YYYY'}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <CalendarComponent
                ...
                <table className="w-full">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 uppercase">PHED Connection</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {submissions.map((sub) => (
                      <tr key={sub.id} className={`hover:bg-slate-50 transition-colors text-sm ${selectedIds.includes(sub.id) ? 'bg-emerald-50' : ''}`}>
                        ...
                        <td className="px-3 py-3" data-testid={`phed-connection-${sub.id}`}>
                          {sub.phed_connection_type && sub.phed_connection_type !== 'UNKNOWN' ? (
                            <ConnectionTypeBadge survey={{ connection_type: sub.phed_connection_type }} testId={`phed-connection-badge-${sub.id}`} />
                          ) : (
                            <span className="text-xs text-slate-400">-</span>
                          )}
                        </td>
                ...
                <div className="border-t pt-2">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    ...
                    <div className="bg-gray-50 rounded p-2">
                      ...
                      <div className="font-medium" data-testid="submission-detail-date-time">{formatISTDateTime(selectedSubmission.submitted_at)}</div>
                    </div>
                  </div>
                </div>
            ...
            {selectedSubmission && (
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between text-sm text-slate-500 pt-2 border-t">
                  ...
                  <span data-testid="submission-print-date-time">Date: <strong>{formatISTDateTime(selectedSubmission.submitted_at)}</strong></span>
