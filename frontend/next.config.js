...
const nextConfig = {
  ...
  // Preview is served from rotating Emergent hosts; allow their /_next/* dev chunk
  // fetches so the client-side dynamic import of the SPA isn't blocked as cross-origin.
  allowedDevOrigins: [
  ...
  env: {
    // Force empty => relative '/api/...' calls (same origin). Hardcoded (not read
    // from .env) so a platform-managed REACT_APP_BACKEND_URL cannot bake a stale
    // cross-origin host into the build. The rewrite below proxies /api to FastAPI,
    // so the app is domain-agnostic and survives any deploy/custom-domain change.
