...
async def import_validate(
    ...
    conn_owner = {(c["service"], c["connection_number_norm"]): c["consumer_id"] for c in existing_conns}

    seen_ids: Dict[str, int] = {}
    seen_conns: Dict[tuple, str] = {}
    ...
    for r in records:
        ...
        for svc, key, ck in (("Water", "water_conn", "new_water_connections"), ("Sewer", "sewer_conn", "new_sewer_connections")):
            if not r[key]:
                continue
            cn = norm_id(r[key])
            owner = conn_owner.get((svc, cn))
            if owner and norm_id(owner) != cid:
                counts["conflicts"] += 1
                conflicts.append({"row": r["row"], "service": svc, "connection_number": r[key], "consumer_id": r["consumer_id"], "existing_consumer_id": owner,
                                  "reason": "Connection number already belongs to a different consumer"})
                continue
            fk = (svc, cn)
            if fk in seen_conns and seen_conns[fk] != cid:
                counts["conflicts"] += 1
                conflicts.append({"row": r["row"], "service": svc, "connection_number": r[key], "consumer_id": r["consumer_id"], "existing_consumer_id": seen_conns[fk],
                                  "reason": "Same connection number used by two consumers in this file"})
                continue
            seen_conns[fk] = cid
            if not owner:
                counts[ck] += 1
        ...
        try:
            ...
            for svc, key in (("Water", "water_conn"), ("Sewer", "sewer_conn")):
                if not r[key] or (r["row"], svc) in conflict_keys:
                    ...
                    continue
                cn = norm_id(r[key])
                if await db.phed_connections.find_one({"service": svc, "connection_number_norm": cn, "is_active": True}, {"_id": 1}):
                    result["skipped_connections"] += 1
                    continue
...
async def _add_connection(consumer_ref: str, consumer: dict, data: ConnectionIn, user: dict, source: str) -> dict:
    ...
    db = get_db()
    dup = await db.phed_connections.find_one({"service": data.service, "connection_number_norm": norm_id(cn), "is_active": True}, {"_id": 0})
    if dup:
        raise HTTPException(409, f"{data.service} connection {cn} already exists for consumer {dup['consumer_id']}")
...
async def save_draft(data: SurveyDraftIn, user: dict = Depends(get_current_user)):
    ...
    if existing and existing["status"] in ("Submitted", "Requires Review", "Approved"):
        raise HTTPException(409, "Survey already submitted for this property")
    ts = now_iso()
...
async def upload_attachment(survey_id: str, attachment_type: str = Form(...), file: UploadFile = File(...), user: dict = Depends(get_current_user)):
    db = get_db()
...
async def delete_attachment(survey_id: str, attachment_id: str, user: dict = Depends(get_current_user)):
    ...
    if survey["surveyor_id"] != user["id"] and not is_admin(user):
        raise HTTPException(403, "Not your survey")
    if survey["status"] in ("Submitted", "Approved") and not is_admin(user):
        raise HTTPException(409, "Survey already submitted")
...
async def submit_survey(survey_id: str, user: dict = Depends(get_current_user)):
    ...
    if survey["surveyor_id"] != user["id"] and not is_admin(user):
        raise HTTPException(403, "Not your survey")
    if survey["status"] in ("Submitted", "Requires Review", "Approved"):
        raise HTTPException(409, "Survey already submitted")
    ...
    if survey["survey_type"] == "WATER_CONNECTION":
        ...
        for cn in conn_numbers:
            if not await db.phed_connections.find_one({"connection_number_norm": norm_id(cn), "service": "Water", "is_active": True}):
    ...
    res = await db.phed_surveys.update_one({"id": survey_id, "status": {"$nin": ["Submitted", "Requires Review", "Approved"]}}, {"$set": upd})
...
async def approve_survey(survey_id: str, user: dict = Depends(get_current_user)):
    ...
    await db.properties.update_one({"id": survey["property_record_id"]}, {"$set": {"phed_survey_status": phed_status}})
    await clear_map_cache()
...
async def list_surveys(status: Optional[str] = None, surveyor_id: Optional[str] = None, ward_id: Optional[str] = None, colony: Optional[str] = None,
                       search: Optional[str] = None, page: int = 1, limit: int = 30, user: dict = Depends(get_current_user)):
    require_officer(user)
    q: Dict[str, Any] = {}
    if status: q["status"] = status
    ...
    if search and search.strip():
        ...
        q["$or"] = [{"reference_number": rx}, {"property_id": rx}]
    total = await get_db().phed_surveys.count_documents(q)
    items = await get_db().phed_surveys.find(q, {"_id": 0}).sort("updated_at", -1).skip((page - 1) * limit).limit(limit).to_list(None)
...
async def reject_survey(survey_id: str, reason: str = Form(...), user: dict = Depends(get_current_user)):
    ...
    await db.properties.update_one({"id": survey["property_record_id"]}, {"$set": {"phed_survey_status": "Rejected"}})
    await clear_map_cache()
...
async def reopen_survey(survey_id: str, reason: str = Form(""), mode: str = Form("correction"), user: dict = Depends(get_current_user)):
    ...
    await db.properties.update_one({"id": survey["property_record_id"]}, {"$set": {"phed_survey_status": "Document Pending"}})
    await clear_map_cache()
