...
const getMarkerColor = (status) => {
  const colors = {
    ...
    'Approved': '#22c55e',     // Green - approved/locked
    'In Progress': '#eab308',  // Same yellow as admin
...
export default function Properties() {
  ...
  const { results: searchResults, loading: searchLoading, error: searchError, page: searchPage, pages: searchPages, loadMore: loadMoreSearch, retry: retrySearch } = useCitywidePropertySearch(searchQuery);
  ...
  useLiveRefresh(() => fetchProperties(true));

  // Always read current survey status; never share cached assignments across users/towns.
  const fetchProperties = async (forceRefresh = false) => {
    ...
    try {
      ...
      setSelectedProperty((current) => current ? props.find((p) => p.id === current.id) || current : null);
      ...
      if (!forceRefresh) toast.success(`Loaded ${props.length} properties`);
    } catch (error) {
      if (requestId !== propertyRequestRef.current) return;
  ...
  // Search the city database rather than the assigned map-marker slice.
  ...
  const selectSearchResult = (property) => {
    ...
    // Center only when this citywide result has a recorded location.
  ...
  const startSurveyForProperty = (property) => {
    ...
    if (property.can_survey === false) {
      toast.info('Survey access is not permitted for this account.');
      return;
    }
    searchInputRef.current?.blur();
    setSelectedProperty(null);
  ...
  useEffect(() => {
    ...
    if (!restoreSearchQuery || restoredSearchRef.current) return;
        ...
        >
          ...
          >
            ...
            <div className="p-4">
              ...
              {selectedProperty.can_survey === false && <p className="mt-3 text-xs text-amber-800" data-testid="citywide-property-read-only">Survey access is not permitted for this account.</p>}
              {/* Action Buttons - Big */}
              <div className="flex gap-3 mt-4">
                {isCompleted(selectedProperty.status) ? (
                  ...
                  <button 
                    disabled={selectedProperty.can_survey === false}
                    className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed text-white px-4 py-4 rounded-xl text-base font-semibold flex items-center justify-center gap-2 shadow-lg"
                ...
                <button 
                  disabled={selectedProperty.latitude == null || selectedProperty.longitude == null}
                  className="bg-gray-100 hover:bg-gray-200 disabled:opacity-40 text-gray-700 px-4 py-4 rounded-xl shadow"
              ...
              {!isCompleted(selectedProperty.status) && selectedProperty.can_edit_location !== false && (
                <button
          ...
          <div className="relative">
            <div className="flex items-center bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              <input
                ...
                placeholder="Search citywide: Property ID, mobile, name..."
                maxLength={200}
            ...
            {showSearchResults && searchResults.length > 0 && (
              ...
              >
                {searchResults.map(withPhedStatus).map((property, index) => (
                  <div
                    key={property.id}
                  ...
                  >
                    <div 
                      ...
                      style={{ backgroundColor: getMarkerColor(property.status) }} data-testid={`search-result-status-color-${property.id}`}
                    >
                ...
                {searchPage < searchPages && <Button variant="ghost" className="w-full" disabled={searchLoading} onClick={loadMoreSearch} data-testid="load-more-property-search">{searchLoading ? 'Loading…' : 'Load more properties'}</Button>}
                {searchError && <Button variant="ghost" className="w-full text-red-700" onClick={retrySearch} data-testid="retry-property-search-page">Search failed. Try again</Button>}
              </div>
            )}
            ...
            {showSearchResults && searchResults.length === 0 && searchQuery.length > 0 && (
              ...
              >
                {searchLoading ? 'Searching city records…' : searchError || `No properties found in this city for "${searchQuery}"`}
                {searchError && <Button variant="ghost" onClick={retrySearch} data-testid="retry-property-search">Try again</Button>}
              </div>
            )}
          </div>
        </div>
