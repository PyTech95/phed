...
import {
  Search, MapPin, Droplet, Waves, Link2, UserPlus, Ban, Crosshair, Camera, Upload,
...
import { ALL_DOCUMENT_STATUSES } from '../../constants/officeDocuments';
import PhedSurveyConsumerList from './PhedSurveyConsumerList';
import { AadhaarCropDialog } from '../../components/AadhaarCropDialog';
import { validateAadhaarPhoto } from '../../lib/aadhaarImage';
...
const ATT_TYPES = [['BILL', 'Water/Sewer Bill'], ['PROPERTY_FRONT', 'Property-front photo'], ['AADHAAR_FRONT', 'Aadhaar — Front'], ['AADHAAR_BACK', 'Aadhaar — Back'], ['AADHAAR', 'Aadhaar (legacy)'], ['REGISTRY', 'Registry (if required)'], ['OTHER', 'Other document']];
...
export default function PhedFieldSurvey() {
  ...
  const [documentStatus, setDocumentStatus] = useState(ALL_DOCUMENT_STATUSES);
  ...
  const loadSurveyConsumers = useCallback(async (pageNumber = 1, append = false) => {
    ...
    try {
      const params = {
        ...
        document_status: documentStatus === ALL_DOCUMENT_STATUSES ? undefined : documentStatus,
      };
  ...
  }, [consumerSearch, pendingOnly, documentStatus]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (isAdmin) return undefined;
    ++consumerRequestRef.current;
    setConsumerRows([]);
    setConsumerLoading(true);
    setConsumerLoadError('');
    setConsumerMeta((current) => ({ ...current, page: 1, pages: 1, total: 0 }));
    const timer = setTimeout(() => loadSurveyConsumers(1, false), 300);
    return () => {
      clearTimeout(timer);
      ++consumerRequestRef.current;
    };
  }, [isAdmin, loadSurveyConsumers]);
  ...
  useEffect(() => {
    if (propertyId && props && !selected && !missingRouteHandledRef.current) {
      ...
      if (p) {
        ...
        axios.get(`${PHED}/property/${propertyId}`, H())
          .then(({ data }) => { setRouteError(''); setSelected(data.property); })
          .catch((error) => setRouteError(error.response?.data?.detail || 'Property नहीं खुली। फिर कोशिश करें।'));
      }
    }
...
function SurveyPanel({ property, onBack, H, isAdmin }) {
  ...
  const attUploadInput = useRef(null);
  const [aadhaarCrop, setAadhaarCrop] = useState(null);
  ...
  const uploadAttachment = async (rawFile, attachmentType = attType) => {
    if (!rawFile) return;
    ...
    try {
      ...
      const fd = new FormData(); fd.append('file', file); fd.append('attachment_type', attachmentType);
      ...
      return true;
    } catch (e) { toast.error(e.response?.data?.detail || e.message || 'Upload failed'); return false; } finally { setUploading(false); if (attInput.current) attInput.current.value = ''; }
  };

  const chooseAttachment = (e) => {
    const file = e.target.files?.[0]; e.target.value = '';
    if (!file) return;
    if (attType.startsWith('AADHAAR') && file.type !== 'application/pdf') {
      try { validateAadhaarPhoto(file); setAadhaarCrop({ file, type: attType }); }
      ...
      <div className="max-w-3xl mx-auto space-y-4 pb-24">
        ...
        <Card className="clinic-card"><CardContent className="p-4">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-3 text-xs" style={{ color: 'var(--phed-muted)' }}>
            ...
            <div>Colony: <span className="text-slate-700">{property.colony || property.ward || '—'}</span></div>
          ...
          {survey && <div className="mt-3"><ConnectionTypeBadge survey={survey} testId="panel-connection-type" /></div>}
          {survey?.submitted_at && <p className="mt-2 text-xs text-slate-600" data-testid="panel-submitted-at">{formatISTDateTime(survey.submitted_at)}</p>}
        ...
        <Card className="clinic-card"><CardContent className="p-4">
          ...
          <div className="mb-2"><OfficeDocumentsStatus water={survey?.water} testId="panel-office-documents-status" /></div>
          <div className="flex flex-wrap gap-2 items-center">
            <Select value={attType} onValueChange={setAttType} disabled={uploading || locked}><SelectTrigger className="w-44" data-testid="att-type-select"><SelectValue /></SelectTrigger><SelectContent data-testid="att-type-options">{ATT_TYPES.map(([v, l]) => <SelectItem key={v} value={v} data-testid={`att-type-${v.toLowerCase()}`}>{l}</SelectItem>)}</SelectContent></Select>
            <input ref={attInput} type="file" accept="image/jpeg,image/png,image/webp" capture="environment" disabled={uploading || locked} className="hidden" onChange={chooseAttachment} data-testid="att-file-input" />
            <input ref={attUploadInput} type="file" accept="image/jpeg,image/png,image/webp,application/pdf" disabled={uploading || locked} className="hidden" onChange={chooseAttachment} data-testid="att-gallery-input" />
            <Button type="button" variant="outline" onClick={() => attInput.current?.click()} disabled={uploading || locked} data-testid="att-upload-btn"><Camera className="w-4 h-4 mr-1.5" />Camera</Button>
            <Button type="button" variant="outline" onClick={() => attUploadInput.current?.click()} disabled={uploading || locked} data-testid="att-gallery-btn">{uploading ? <Loader2 className="w-4 h-4 mr-1.5 animate-spin" /> : <Upload className="w-4 h-4 mr-1.5" />}Upload</Button>
          </div>
          {aadhaarCrop && <AadhaarCropDialog file={aadhaarCrop.file} side={aadhaarCrop.type === 'AADHAAR_FRONT' ? 'front' : aadhaarCrop.type === 'AADHAAR_BACK' ? 'back' : 'photo'} onCancel={() => setAadhaarCrop(null)} onSave={async (file) => {
            if (await uploadAttachment(file, aadhaarCrop.type)) setAadhaarCrop(null);
            else throw new Error('Upload failed. Your crop is still here — please retry.');
          }} />}
