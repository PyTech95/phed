...
export default function SelectTown() {
  ...
  // Auto-forward: a user with exactly one accessible town should never have to
  // manually pick it. This also covers the brief first-login bounce here from
  // ProtectedRoute before TownContext finishes auto-selecting.
  useEffect(() => {
    if (townsLoading || selectedId) return;
    const accessible = user?.role === 'ADMIN'
      ? towns
      : towns.filter(t => !user?.assigned_town || t.id === user.assigned_town);
    if (accessible.length === 1) {
      selectTown(accessible[0]);
      const destination = (user?.role === 'ADMIN' || user?.role === 'SUPERVISOR' || user?.role === 'MC_OFFICER') ? '/admin' : '/employee';
      navigate(destination, { replace: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [townsLoading, towns, user, selectedId]);
