import { Loader2, Lock, MapPin, Search, UserRoundPlus } from 'lucide-react';
  ...
  return (
    <EmployeeLayout title="Water Supply Survey">
      <div className="mx-auto max-w-md" data-testid="phed-survey-consumer-list">
        ...
        <div className="mb-4">
          <OfficeDocumentStatusFilter value={documentStatus} onChange={onDocumentStatusChange} testId="staff-document-status-filter" />
        </div>
        <div className="mb-4 flex gap-2 text-xs">
          ...
          <p className="mb-3 text-xs text-slate-500" data-testid="staff-filtered-record-count">{loading ? 'Loading matching records…' : `${stats.total ?? 0} matching records`}</p>
        ...
        <div className="space-y-2.5">
          {rows.map((consumer) => {
            ...
            const approved = consumer.survey_status === 'Approved';
            return (
              <button
                type="button"
                key={consumer.id}
                ...
                className={`clinic-card w-full cursor-pointer border p-4 text-left transition-shadow hover:shadow-md ${
                  approved ? 'border-green-300 bg-green-50' : isLinked ? 'border-yellow-300 bg-yellow-50' : ''
                }`}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1.5">
                      <ConnectionAvailability
                        ...
                        testId={`phed-consumer-availability-${consumer.id}`}
                      />
                    </div>
                  ...
                  <Badge
                    ...
                    className={`shrink-0 text-[10px] ${
                      approved ? 'border-green-400 bg-green-100 text-green-800' : 'border-amber-300 text-amber-700'
                    }`}
                    data-testid={`phed-consumer-link-status-${consumer.id}`}
                  >
                    {approved ? <><Lock className="mr-1 inline h-3 w-3" />Approved</> : isLinked ? consumer.survey_status === 'Not Started' ? 'Linked · Survey pending' : consumer.survey_status : 'Select Property ID'}
                  </Badge>
                </div>
                {consumer.office_document_receipt && (
                  <div className="mt-3 rounded-md bg-teal-50 p-2 text-xs text-teal-800" data-testid={`phed-consumer-document-receipt-${consumer.id}`}>
                    <p className="font-semibold" data-testid={`phed-consumer-document-status-${consumer.id}`}>{consumer.office_document_receipt.status}</p>
                    <p className="mt-1 font-mono" data-testid={`phed-consumer-document-property-${consumer.id}`}>Property ID: {consumer.office_document_receipt.property_id}</p>
                    {consumer.office_document_receipt.remarks && <p className="mt-1 whitespace-pre-wrap" data-testid={`phed-consumer-document-remarks-${consumer.id}`}>{consumer.office_document_receipt.remarks}</p>}
                  </div>
                )}
              </button>
            );
