...
export default function PhedFieldSurvey() {
  ...
  const filtered = (props || []).filter((p) => {
    ...
    return [p.property_id, p.owner_name, p.address, p.colony, p.mobile].some((v) => (v || '').toLowerCase().includes(s));
    ...
    <EmployeeLayout title={isAdmin ? 'PHED Consumer Survey' : 'Water Supply Survey'}>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="relative mb-2">
          ...
          <Input className="pl-9 h-11" placeholder="Search property ID, owner, mobile, address…" value={q} onChange={(e) => setQ(e.target.value)} data-testid="field-property-search" />
        </div>
        {!isAdmin && (
