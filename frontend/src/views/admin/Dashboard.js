          ...
          <div className="flex items-center gap-3">
            ...
            {viewMode === 'today' && (
              <div className="w-44 -mt-1">
                <IndianDateInput testId="date-picker" value={selectedDate} onChange={setSelectedDate} label="Report date" />
              </div>
            )}
            {isAdmin && (
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <IndianDateInput testId="report-filter-date-from" value={reportFilters.date_from}
                  onChange={(v) => setReportFilters(f => ({...f, date_from: v}))} label="Date from" />
              </div>
              <div>
                <Label className="text-xs font-medium text-slate-600 mb-1 block">Date To</Label>
                <IndianDateInput testId="report-filter-date-to" value={reportFilters.date_to}
                  onChange={(v) => setReportFilters(f => ({...f, date_to: v}))} label="Date to" />
              </div>
            </div>
