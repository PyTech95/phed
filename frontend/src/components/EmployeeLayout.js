...
const navItems = [
  ...
  { path: '/employee/submissions', icon: ClipboardCheck, label: 'Submissions' },
];
...
export default function EmployeeLayout({ children, title, showBackButton = false }) {
  ...
  return (
    <div className="min-h-screen pb-20" style={{background: 'var(--phed-bg)'}}>
      {/* Header */}
      {location.pathname !== '/employee/properties' && <header className="surveyor-header sticky top-0 z-30 border-b" style={{background: 'rgba(255,255,255,0.96)', backdropFilter: 'blur(12px)', borderColor: 'var(--phed-border)'}}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex min-w-0 flex-col items-center gap-1 px-2 py-2 rounded-xl transition-colors ${
                isActive 
                  ? 'text-blue-700' 
              ...
              data-testid={`nav-tab-${
                item.path === '/employee'
                  ...
                  : item.path === '/employee/properties'
                    ? 'properties'
                    : item.path === '/employee/submissions' ? 'submissions' : 'water-survey'
              }`}
            >
