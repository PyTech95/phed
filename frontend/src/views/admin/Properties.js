...
export default function Properties() {
  ...
  const colonyOptions = (allColonies || []).filter((c) => c.toLowerCase().includes(colonySearch.trim().toLowerCase()));
            ...
            <div className="flex flex-wrap gap-3 items-end">
              ...
              >
                <SelectTrigger className="w-[150px]" data-testid="area-filter">
                  <SelectValue placeholder="All Areas/Colonies" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value=" ">All Areas/Colonies</SelectItem>
                  {areas.map(a => (
            ...
            <div className="space-y-4 py-4">
              {selectedProperties.length === 0 && (
                <div className="space-y-2">
                  <label className="text-sm font-medium">Area / Colony</label>
                  <Select 
                  ...
                  >
                    <SelectTrigger data-testid="bulk-area-select">
                      <SelectValue placeholder="Select area / colony" />
                    </SelectTrigger>
                    <SelectContent>
                      {areas.map(a => (
                        <SelectItem key={a} value={a}>{a}</SelectItem>
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  {colonyOptions.length > 0 && (
                    <button type="button" className="text-xs text-blue-600 underline" onClick={() => setSelectedColonies(selectedColonies.length === colonyOptions.length ? [] : [...colonyOptions])} data-testid="assign-select-all-colonies">
                      {selectedColonies.length === colonyOptions.length ? 'Clear all' : 'Select all'}
                    </button>
                  )}
                </div>
                <input
                  className="w-full h-10 border rounded-lg px-3 mt-2 text-sm"
                  placeholder="Colony खोजें…"
                  value={colonySearch}
                  onChange={(e) => setColonySearch(e.target.value)}
                  data-testid="assign-colony-search"
                />
                <div className="max-h-40 overflow-y-auto border rounded-lg p-2 mt-2 space-y-1">
                  {colonyOptions.length === 0 ? (
                    <p className="text-sm text-slate-400 py-2 text-center">{coloniesLoading ? 'Loading colonies...' : 'कोई colony नहीं मिली'}</p>
