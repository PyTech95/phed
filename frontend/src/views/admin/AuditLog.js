...
function formatTs(ts) {
  try { return formatISTDateTime(ts); } catch { return ts; }
}
        ...
        <Card className="border" style={{ borderColor: 'var(--phed-border)' }}>
          <CardContent className="p-4 grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
            ...
            <div>
              ...
              <IndianDateInput testId="audit-date-from" value={filters.date_from} onChange={(v) => setFilter('date_from', v)} label="From date" />
            </div>
            <div>
              <label className="text-xs font-medium block mb-1" style={{ color: 'var(--phed-muted)' }}>To</label>
              <IndianDateInput testId="audit-date-to" value={filters.date_to} onChange={(v) => setFilter('date_to', v)} label="To date" />
            </div>
          </CardContent>
        </Card>
