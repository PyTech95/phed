...
// 3-colour PHED survey rule: red = not surveyed, yellow = surveyor submitted (reopenable), green = admin approved.
// Overrides the property's `status` with the survey-derived one so markers, stats, legend and the
// status filter all reflect the PHED survey (not the house-tax status). Original kept as `mc_status`.
const withSurveyStatus = (p) => {
  ...
  const status = st === 'Approved' ? 'Approved'
    ...
    : 'Pending';
  return { ...p, mc_status: p.status, status };
};

const surveyColorOf = (status) => (status === 'Approved' ? 'green' : status === 'Completed' ? 'yellow' : 'red');
...
export default function PropertyMap() {
  ...
  const saveMove = async () => {
    ...
    setSavingMove(true);
    try {
      await axios.put(`${API_URL}/phed/properties/${moved.id}/location`,
        { latitude: moved.latitude, longitude: moved.longitude }, { headers: { Authorization: `Bearer ${token}` } });
      const patch = (ps) => ps.map((p) => (p.id === moved.id ? { ...p, latitude: moved.latitude, longitude: moved.longitude } : p));
      setProperties(patch); setFilteredProperties(patch);
      toast.success('Pin location saved');
      setMoved(null);
    } catch (e) { toast.error(e.response?.data?.detail || 'Save failed'); } finally { setSavingMove(false); }
  };
  ...
  const [summary, setSummary] = useState(null); // town-wide {red,yellow,green,total} for legend
  ...
  useEffect(() => {
    ...
    fetchSummary(); // Town-wide red/yellow/green counts for the legend
  ...
  useEffect(() => {
    ...
    fetchSummary(filters.colony && filters.colony !== 'ALL_AREAS' ? filters.colony : '');
  }, [filters.colony]);

  const fetchSummary = async (colony = '') => {
    try {
      const { data } = await axios.get(`${API_URL}/phed/map-summary`, {
        headers: { Authorization: `Bearer ${token}` }, params: colony ? { colony } : {},
      });
      setSummary(data);
    } catch { /* ignore */ }
  };
  ...
  const fetchPropertiesByColony = async (colony) => {
    ...
    try {
      ...
      props = props.map(withSurveyStatus);
      ...
      setStats({
        ...
        // Survey status (PHED, survey-derived)
        ...
        // Survey OUTCOME breakdown (PHED)
        newConnection: props.filter(p => p.phed_outcome === 'NEW').length,
        propertyLocked: props.filter(p => p.phed_outcome === 'LOCKED').length,
        ownerDenied: props.filter(p => p.phed_outcome === 'DENIED').length,
        hasConnection: props.filter(p => p.phed_outcome === 'HAS_CONNECTION').length,
        // Submission based
      ...
      <div data-testid="property-map-page" className="space-y-4">
        ...
        {showMap && (
          <>
            ...
            {/* Always-visible town-wide survey progress (no area selection needed). Click a colour to filter pins. */}
            {summary && (
              <div className="flex flex-wrap items-center gap-2 text-xs bg-white border rounded-lg px-3 py-2 mb-2" data-testid="map-legend-counts">
                <span className="font-semibold text-slate-600">{filters.colony && filters.colony !== 'ALL_AREAS' ? filters.colony : 'Town'} survey status:</span>
                {[['red', 'Pending', summary.red, 'bg-red-500', 'text-red-600'],
                  ['yellow', 'Surveyed', summary.yellow, 'bg-yellow-500', 'text-yellow-600'],
                  ['green', 'Approved', summary.green, 'bg-green-500', 'text-green-600']].map(([key, label, val, dot, txt]) => (
                  <button key={key} type="button" onClick={() => setSurveyFilter((s) => (s === key ? null : key))}
                    data-testid={`legend-filter-${key}`}
                    className={`flex items-center gap-1 rounded-full px-2 py-0.5 border transition-colors ${surveyFilter === key ? 'border-slate-800 bg-slate-100' : 'border-transparent hover:bg-slate-50'}`}>
                    <div className={`w-3 h-3 rounded-full ${dot}`}></div> {label} <b className={txt}>{val}</b>
                  </button>
                ))}
                <span className="text-slate-500">Total <b>{summary.total}</b></span>
                {surveyFilter && (
                  <button type="button" onClick={() => setSurveyFilter(null)} className="ml-1 text-slate-500 underline" data-testid="legend-filter-clear">clear filter</button>
                )}
              </div>
            )}
            ...
            {filters.colony && (
              <div className="space-y-2">
                ...
                {/* Row 2: PHED survey OUTCOME stats (not house-tax) */}
                <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-8 gap-2">
                  <div className="bg-green-50 border border-green-200 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-green-700">{stats.completed + stats.approved}</p>
                    <p className="text-xs text-green-600">Survey Done</p>
                  </div>
                  <div className="bg-red-50 border border-red-200 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-red-700">{stats.pending}</p>
                    <p className="text-xs text-red-600">Not Surveyed</p>
                  </div>
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-blue-700">{stats.hasConnection}</p>
                    <p className="text-xs text-blue-600">Has Connection</p>
                  </div>
                  <div className="bg-violet-50 border border-violet-200 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-violet-700">{stats.newConnection}</p>
                    <p className="text-xs text-violet-600">New Connection</p>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-slate-700">{stats.propertyLocked}</p>
                    <p className="text-xs text-slate-600">Property Locked</p>
                  </div>
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-amber-700">{stats.ownerDenied}</p>
                    <p className="text-xs text-amber-600">Owner Denied</p>
                  </div>
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-orange-700">{stats.rejected}</p>
              ...
              <CardContent className="py-4">
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-x-3 gap-y-2 text-sm">
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs" data-testid="map-legend-inline">
                    ...
                    <span className="flex items-center gap-1">
                      <div className="w-3 h-3 rounded-full bg-yellow-500"></div> Surveyed {summary ? <b className="text-yellow-600">{summary.yellow}</b> : ''}
                    </span>
                    <span className="flex items-center gap-1">
                      <div className="w-3 h-3 rounded-full bg-green-500"></div> Approved {summary ? <b className="text-green-600">{summary.green}</b> : ''}
                    </span>
                    {summary ? <span className="text-slate-500">Total <b>{summary.total}</b></span> : null}
                  </div>
                </div>
            ...
            <div className="mt-4 pt-4 border-t flex flex-wrap items-center gap-3">
              {moved && (
                <div className="w-full flex flex-wrap items-center gap-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2" data-testid="admin-move-bar">
                  <span className="text-sm text-amber-900 flex-1">
                    Pin <strong>{moved.property_id}</strong> moved to <span className="font-mono text-xs">{moved.latitude.toFixed(6)}, {moved.longitude.toFixed(6)}</span>
                  </span>
                  <Button size="sm" variant="outline" onClick={() => setMoved(null)} data-testid="admin-move-cancel-btn">Cancel</Button>
                  <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={saveMove} disabled={savingMove} data-testid="admin-move-save-btn">
                    {savingMove ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-1" /> Save location</>}
                  </Button>
                </div>
              )}
              <Button
                ...
                >
                  ...
                  {spreadOverlappingMarkers(surveyFilter ? filteredProperties.filter((p) => surveyColorOf(p.status) === surveyFilter) : filteredProperties).map((property) => (
                    <Marker
                      ...
                      draggable
                      eventHandlers={{ dragend: (e) => { const { lat, lng } = e.target.getLatLng(); setMoved({ id: property.id, property_id: property.property_id, latitude: lat, longitude: lng }); } }}
                ...
                <div className="border-t pt-2">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    ...
                    <div className="bg-slate-50 rounded p-2">
                      ...
                      <div className="font-medium text-[10px]" data-testid="map-submission-date-time">{formatISTDateTime(surveyData.submitted_at)}</div>
                    </div>
                  </div>
                </div>
