...
import PhedLinkCleanup from "./views/admin/PhedLinkCleanup";
import MySubmissions from "./views/employee/MySubmissions";
...
axios.interceptors.request.use((config) => {
  // Identity discovery is global; stale town selection must not block login/me.
  if (config.url?.includes('/auth/')) return config;
  try {
    const savedTown = localStorage.getItem('selectedTown');
    if (savedTown) {
      const town = JSON.parse(savedTown);
      if (town && town.code && !config.headers['X-Town-Code']) {
...
function AppRoutes() {
  ...
  return (
    <Routes>
      ...
      <Route path="/admin/notice-submissions" element={
        <ProtectedRoute allowedRoles={ADMIN_VIEW_ROLES}>
          <AdminSubmissions />
        </ProtectedRoute>
      } />
      ...
      <Route path="/admin/phed/link-cleanup" element={
        <ProtectedRoute allowedRoles={ADMIN_ROLES}>
          <PhedLinkCleanup />
        </ProtectedRoute>
      } />
