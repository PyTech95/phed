...
from survey_presentation import connection_code as phed_connection_code  # noqa: E402
...
async def startup_event():
    ...
    await seed_defaults()
    # Ensure per-town indexes (incl. PHED + import-journal) exist for every town.
    # Towns seeded directly bypass create_town_indexes, so build them here idempotently.
    try:
        for town in await master_db.towns.find({}, {"_id": 0, "code": 1}).to_list(None):
            if town.get("code"):
                await create_town_indexes(get_town_db(town["code"]))
    except Exception as e:
        logging.getLogger("app").warning(f"town index ensure warning: {e}")
...
class UserCreate(BaseModel):
    ...
    extra_town_ids: Optional[List[str]] = None  # Additional towns this user may also access
...
class UserResponse(BaseModel):
    ...
    extra_town_ids: Optional[List[str]] = None
...
async def get_accessible_towns(user: dict) -> List[dict]:
    """Towns this user may work in: primary assigned town + any extra grants."""
    town_ids = []
    if user.get("assigned_town"):
        town_ids.append(user["assigned_town"])
    town_ids += [t for t in (user.get("extra_town_ids") or []) if t]
    if not town_ids:
        return []
    return await master_db.towns.find({"id": {"$in": town_ids}, "is_active": True}, {"_id": 0}).to_list(None)
...
async def get_current_user(authorization: str = Header(None)):
    ...
    # Non-admin users may only act inside towns they can access (assigned + extra grants)
    if user.get("role") != "ADMIN":
        accessible = await get_accessible_towns(user)
        allowed_codes = {t["code"].upper() for t in accessible if t.get("code")}
        if allowed_codes and get_current_town_code().upper() not in allowed_codes:
...
async def login(request: Request, data: UserLogin):
    ...
    if user["role"] == "ADMIN":
        ...
        # Assigned town + any extra granted towns
        user_towns = await get_accessible_towns(user)
...
async def get_me(current_user: dict = Depends(get_authenticated_user)):
    ...
    if role == "ADMIN":
        ...
        user_towns = await get_accessible_towns(current_user)
...
async def get_colonies_list(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    pipeline = [
        ...
        {"$group": {"_id": {"$cond": [{"$or": [{"$eq": ["$colony", None]}, {"$eq": ["$colony", ""]}]}, "$ward", "$colony"]}, "count": {"$sum": 1}}},
        {"$match": {"_id": {"$ne": None}}},
        {"$sort": {"_id": 1}}
    ]
...
async def get_map_properties(
    ...
    await enrich_survey_states(get_db(), properties)
...
from property_map import EmployeeMapResponse, normalize_location


@api_router.get("/map/employee-properties", response_model=EmployeeMapResponse)
async def get_employee_map_properties(
    hide_completed: bool = False,
    current_user: dict = Depends(get_current_user)
):
    """All assigned work in this town, including records awaiting GPS capture."""
    
    # PHED state is live; a cached snapshot must not hide approvals or returns.
    ...
    # Apply hide_completed after resolving the current PHED survey state.
    ...
    from survey_state import enrich_survey_states
    await enrich_survey_states(get_db(), unique_properties)
    if hide_completed:
        unique_properties = [p for p in unique_properties if p.get("phed_survey_state") != "Approved"]
    mapped_count = sum(normalize_location(p) for p in unique_properties)
    result = {
        "properties": unique_properties,
        "count": len(unique_properties),
        "mapped_count": mapped_count,
        "missing_location_count": len(unique_properties) - mapped_count,
    }
...
async def create_user(data: UserCreate, request: Request, current_user: dict = Depends(get_current_user)):
    ...
    # Extra town grants: keep only real towns, and not the primary assigned town
    extra_town_ids = []
    if data.extra_town_ids:
        valid = await master_db.towns.find({"id": {"$in": data.extra_town_ids}}, {"_id": 0, "id": 1}).to_list(None)
        extra_town_ids = [t["id"] for t in valid if t["id"] != assigned_town]
    ...
    return {
        ...
        "extra_town_ids": user_doc["extra_town_ids"],
...
async def update_user(user_id: str, data: UpdateUserRequest, request: Request, current_user: dict = Depends(get_current_user)):
    ...
    if data.extra_town_ids is not None:
        valid = await master_db.towns.find({"id": {"$in": data.extra_town_ids}}, {"_id": 0, "id": 1}).to_list(None)
        update_data["extra_town_ids"] = [t["id"] for t in valid if t["id"] != user.get("assigned_town")]
...
async def upload_batch(
    ...
    if filename.endswith('.xlsx') or filename.endswith('.xls'):
        ...
        # Create header mapping (normalized: lowercase, alphanumeric only —
        # so "COLONY NAME", "colony_name", "Colony  Name" all match)
        def _norm_h(s):
            return re.sub(r'[^a-z0-9]', '', str(s).lower())
        header_map = {_norm_h(h): i for i, h in enumerate(headers)}
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            prop = {
                ...
                "property_id": get_val(["Property Id", "property_id", "PropertyID", "PID"]) or str(uuid.uuid4())[:8].upper(),
                "old_property_id": get_val(["Old Property Id", "old_property_id", "OldPropertyId"]),
                "owner_name": get_val(["Owner Name", "owner_name", "OwnerName", "Owner", "ONWER"]) or "Unknown",
                "mobile": get_val(["Mobile", "mobile", "Mobile No", "Phone"]),
                "address": get_val(["Plot Address", "Address", "address", "plot_address"]),
                "colony": get_val(["Colony", "colony", "Colony Name", "colony_name", "Locality", "Area", "area"]),
                "ward": (get_val(["Ward No", "Ward Number", "Ward", "ward", "ward_no"])
                         or ward_master.ward_for_colony(get_val(["Colony", "colony", "Colony Name", "colony_name", "Locality", "Area", "area"]))
                         or get_val(["Colony", "colony", "Colony Name", "colony_name", "Locality", "Area", "area"])),
                "latitude": None,
                "longitude": None,
        ...
        for row in reader:
            prop = {
                "id": str(uuid.uuid4()),
                "serial_number": serial_num,
                "property_id": row.get("property_id") or row.get("Property Id") or row.get("PropertyID") or row.get("PID") or str(uuid.uuid4())[:8].upper(),
                "old_property_id": row.get("old_property_id") or row.get("Old Property Id") or "",
                "owner_name": row.get("owner_name") or row.get("Owner Name") or row.get("OwnerName") or row.get("Owner") or row.get("ONWER") or "Unknown",
                ...
                "colony": row.get("Colony") or row.get("colony") or row.get("Colony Name") or row.get("colony_name") or row.get("Locality") or row.get("Area") or "",
                "ward": (row.get("Ward No") or row.get("Ward Number") or row.get("ward") or row.get("Ward")
                         or ward_master.ward_for_colony(row.get("Colony") or row.get("colony") or row.get("Colony Name") or row.get("Locality") or row.get("Area") or "")
                         or row.get("Colony") or row.get("colony") or row.get("Colony Name") or ""),
                "latitude": None,
                "longitude": None,
...
async def list_properties(
    ...
    if ward and ward.strip():
        query["$and"] = query.get("$and", []) + [{"$or": [{"ward": ward}, {"colony": ward}]}]
...
async def bulk_assign_by_ward(data: BulkAssignmentRequest, current_user: dict = Depends(get_current_user)):
    ...
    props_count = await get_db().properties.count_documents({"$or": [{"ward": data.area}, {"colony": data.area}]})
    ...
    # Build query - match area against both ward and colony, serial filtering done in Python
...
async def get_employee_progress(
    ...
    if town_code:
        town = await master_db.towns.find_one({"code": town_code}, {"_id": 0})
        if town:
            emp_filter["$or"] = [{"assigned_town": town["id"]}, {"extra_town_ids": town["id"]}]
...
async def list_areas(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    town_db = await get_town_data_db(request)
    areas = await _distinct_area_names(town_db.properties)
    result = {"areas": areas}
...
async def list_submissions(
    ...
    # Latest PHED survey connection type (water/sewer) per property, for the list's PHED Connection column
    phed_conn_map = {}
    if prop_ids:
        phed_rows = await get_db().phed_surveys.find(
            {"property_record_id": {"$in": prop_ids}, "submitted_at": {"$ne": None}},
            {"_id": 0, "property_record_id": 1, "water": 1, "survey_type": 1, "submitted_at": 1}
        ).sort("submitted_at", -1).to_list(None)
        for row in phed_rows:
            key = row.get("property_record_id")
            if key and key not in phed_conn_map:
                phed_conn_map[key] = phed_connection_code(row)

    for sub in submissions:
        prop = props_map.get(sub.get("property_record_id"))
        sub["phed_connection_type"] = phed_conn_map.get(sub.get("property_record_id"))
        if prop:
            pid = prop.get("property_id", "")
...
class PropertySearchItem(BaseModel):
    """Public search projection, not the editable property document."""
...
@api_router.get("/employee/properties/search")
async def search_city_properties(
    ...
    """Citywide survey lookup. Maps and master property edits remain assigned-only."""
    term = search.strip()
    if not term:
        return {"properties": [], "total": 0, "page": page, "pages": 0}
    pattern = re.escape(term)
    query = {"$or": [
        {field: {"$regex": pattern, "$options": "i"}}
        for field in ("property_id", "owner_name", "mobile", "address", "colony")
    ]}
    # Serial numbers can be numeric in imported property-tax records.
    query["$or"].extend({"$expr": {"$regexMatch": {
        "input": {"$convert": {"input": f"${field}", "to": "string", "onError": "", "onNull": ""}},
        "regex": pattern, "options": "i",
    }}} for field in ("bill_sr_no", "serial_number"))
    projection = {"_id": 0, **{field: 1 for field in (
        "id", "property_id", "owner_name", "mobile", "address", "colony", "ward",
        "latitude", "longitude", "status", "serial_number", "bill_sr_no", "category",
        "total_area", "phed_survey_status", "phed_survey_state", "phed_outcome", "assigned_employee_id", "assigned_employee_ids",
    )}}
    city_db = get_db()
    total = await city_db.properties.count_documents(query)
    docs = await city_db.properties.find(query, projection).sort([
        ("property_id", 1), ("id", 1),
    ]).skip((page - 1) * limit).limit(limit).to_list(limit)
    results = []
    from survey_state import enrich_survey_states
    await enrich_survey_states(city_db, docs)
    for doc in docs:
        can_survey = (
            current_user["role"] in ADMIN_VIEW_ROLES
            or current_user["role"] in ("EMPLOYEE", "SURVEYOR")
        )
        doc["can_edit_location"] = current_user["role"] in ADMIN_VIEW_ROLES or doc.get("assigned_employee_id") == current_user["id"] or current_user["id"] in (doc.get("assigned_employee_ids") or [])
        doc.pop("assigned_employee_id", None)
        doc.pop("assigned_employee_ids", None)
        doc["can_survey"] = can_survey
        results.append(PropertySearchItem.model_validate(doc))
    return {"properties": results, "total": total, "page": page, "pages": (total + limit - 1) // limit}
...
async def get_employee_properties(
    ...
    if search:
        ...
        query["$and"] = query.get("$and", []) + [{
            "$or": [
                {field: {"$regex": pattern, "$options": "i"}}
                for field in ("property_id", "owner_name", "mobile", "colony", "address")
            ]
        }]
...
async def get_all_colonies(current_user: dict = Depends(get_current_user)):
    ...
    colonies = await _distinct_area_names(get_db().properties)
