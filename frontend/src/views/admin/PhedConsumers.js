...
import {
  Search, Droplet, Waves, Link2, Unlink, Plus, Eye, Download, ChevronLeft, ChevronRight, Loader2, Users, X, Building2,
} from 'lucide-react';
              ...
              <TableBody>
                ...
                {rows?.map((c) => (
                  <TableRow key={c.id} data-testid={`consumer-row-${c.id}`}>
                    ...
                    <TableCell><Button size="sm" variant="ghost" onClick={() => openDetail(c.id)} data-testid={`view-consumer-${c.id}`}><Eye className="w-4 h-4" /></Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
