...
from survey_documents import office_declaration_water, validate_office_submission
from import_safety import explicit_id, import_write, batch_document, deletion_plan, undo_batch
from survey_presentation import CONNECTION_LABELS, IST, connection_code, connection_query, indian_date_range, ist_datetime_text
...
HEADER_ALIASES = {
    ...
    "property_id": ["property id", "property id pid", "pid", "mc property id", "property no"],
...
FIELD_LABELS = {
    ...
    "sewer_conn": "Sewer Connection No.", "category": "Type of Connection", "property_id": "Property ID",
}
REQUIRED_FIELDS = ["consumer_name", "consumer_id"]


OfficeDocumentStatus = Literal[
...
OfficeDocumentFilter = Union[OfficeDocumentStatus, Literal["not_submitted"]]


def office_document_query(document_status: Optional[OfficeDocumentFilter]) -> dict:
    if document_status is None:
        return {}
    return {"office_document_receipt.status": None if document_status == "not_submitted" else document_status}
...
async def surveyor_can_access_property(user: dict, prop: dict) -> bool:
    # Authentication has already enforced the selected town. Map scope is separate.
    return is_officer(user) or user.get("role") in ("EMPLOYEE", "SURVEYOR")
...
async def get_property_or_403(property_record_id: str, user: dict) -> dict:
    ...
    if not await surveyor_can_access_property(user, prop):
        raise HTTPException(403, "Survey access is not permitted")
    return prop
...
async def ensure_indexes(db):
    await db.phed_consumers.create_index("id", background=True)
    ...
    await db.phed_consumers.create_index([("office_document_receipt.status", 1), ("ward_number", 1), ("consumer_id", 1)], background=True)
    await db.phed_connections.create_index("id", background=True)
    ...
    await db.phed_surveys.create_index("id", background=True)
    await db.phed_surveys.create_index([("property_record_id", 1), ("status", 1)], background=True)
    await db.phed_surveys.create_index("consumer_ref", background=True)
    await db.phed_surveys.create_index("consumer_refs", background=True)
    await db.phed_surveys.create_index("office_batch_id", background=True)
    ...
    # Import journal: reversible-import lookups must be indexed or delete/preview
    # table-scans the whole journal (slow/timeout on large imports).
    await db.phed_import_changes.create_index([("batch_id", 1), ("undone", 1)], background=True)
    await db.phed_import_changes.create_index([("batch_id", 1), ("collection", 1), ("document_id", 1)], unique=True, background=True)
    await db.phed_import_changes.create_index("id", background=True)
    await db.phed_consumers.create_index("import_id", background=True)
    await db.phed_connections.create_index("import_id", background=True)
...
def row_to_record(mapping, row_tuple):
    ...
    rec = {
        ...
        "address": g("address"), "locality": g("locality"), "phone": g("phone"), "consumer_id": explicit_id(g("consumer_id")),
        "property_id": explicit_id(g("property_id")),
        "water_conn": g("water_conn"), "sewer_conn": g("sewer_conn"), "category_raw": g("category"), "category": cat,
        "category_known": known,
    }
    errors, warnings = [], []
    if not rec["consumer_id"] or not norm_id(rec["consumer_id"]):
...
async def import_validate(
    ...
    # Ward is optional now; colony drives grouping (from the sheet's Locality column or default_colony).
    ward = None
    if ward_id:
    ...
    for r in records:
        if r["property_id"]:
            matches = await db.properties.find({"property_id": {"$regex": f"^{re.escape(r['property_id'])}$", "$options": "i"}}, {"_id": 0, "id": 1}).limit(2).to_list(None)
            if len(matches) != 1:
                r["errors"].append("Property ID not found or ambiguous; row will not be linked")
    ...
    doc = {
        "id": import_id, "filename": re.sub(r"[^A-Za-z0-9._ -]", "_", file.filename or "upload.xlsx"), "ward_id": (ward or {}).get("id"), "ward_number": (ward or {}).get("ward_number"),
        "default_colony": default_colony.strip(), "mode": mode, "uploaded_by": user["id"], "uploaded_by_name": user.get("name"),
        "status": "Validated", "rollback_version": 1, "header_mapping": {FIELD_LABELS[f]: raw_headers[i] for f, i in mapping.items()}, "unmapped_headers": unmapped,
    ...
    doc.pop("_id", None)
    preview = [{k: r[k] for k in ("row", "consumer_id", "property_id", "consumer_name", "fh_name", "ppp_id", "address", "locality", "phone", "water_conn", "sewer_conn", "category", "warnings", "errors")} for r in records[:10]]
...
async def _run_import(import_id: str, user: dict):
    ...
    for r in records:
        try:
            ...
            if not explicit_id(r["consumer_id"]) or not cid_norm:
                raise ValueError("Consumer ID is required; name/phone matching is not allowed")
            existing = await db.phed_consumers.find_one({"consumer_id_norm": cid_norm}, {"_id": 0})
            prop = None
            if r.get("property_id"):
                matches = await db.properties.find({"property_id": {"$regex": f"^{re.escape(r['property_id'])}$", "$options": "i"}}, {"_id": 0}).limit(2).to_list(None)
                if len(matches) != 1:
                    raise ValueError("Property ID not found or ambiguous")
                prop = matches[0]
                if existing and existing.get("linked_property_id") not in (None, prop["id"]):
                    raise ValueError("Consumer is already linked to a different Property ID")
            ...
            if existing:
                if mode == "new_only":
                    ...
                    if prop:
                        keep.update(linked_property_id=prop["id"], linked_property_number=prop["property_id"])
                    await import_write(db, import_id, "phed_consumers", existing["id"], {**base, **keep})
                ...
                await import_write(db, import_id, "phed_consumers", consumer_ref, {**base, "id": consumer_ref, "linked_property_id": (prop or {}).get("id"), "linked_property_number": (prop or {}).get("property_id"),
                                                    "status": "Imported", "is_active": True, "provisional_ref": None, "created_at": ts})
                result["created_consumers"] += 1
            if prop:
                connections = await db.phed_connections.find({"consumer_ref": consumer_ref, "is_active": True}, {"_id": 0}).to_list(None)
                for connection in connections:
                    await import_write(db, import_id, "phed_connections", connection["id"], {"linked_property_id": prop["id"], "updated_at": ts})
            for svc, key in (("Water", "water_conn"), ("Sewer", "sewer_conn")):
                ...
                await import_write(db, import_id, "phed_connections", connection_id, {
                    "id": connection_id, "consumer_ref": consumer_ref, "consumer_id": r["consumer_id"], "connection_number": r[key],
                    "connection_number_norm": cn, "service": svc, "category": r["category"], "category_raw": r["category_raw"], "source": "imported",
                    "status": "Active", "is_active": True, "linked_property_id": (prop or {}).get("id") or (existing or {}).get("linked_property_id"), "import_id": import_id, "created_at": ts, "updated_at": ts,
                })
...
async def import_commit(import_id: str, user: dict = Depends(get_current_user)):
    ...
    claimed = await get_db().phed_imports.update_one({"id": import_id, "status": "Validated"}, {"$set": {"status": "Processing"}})
    if not claimed.modified_count:
        raise HTTPException(409, "Import is already processing")
...
class ImportDeletionPreview(BaseModel):
    filename: str
    status: str
    remove: int
    restore: int
...
class ImportDeleteIn(BaseModel):
    confirmation: str
    force: bool = False
...
@phed_router.get("/import-batches/{kind}/{batch_id}/deletion-preview", response_model=ImportDeletionPreview)
async def preview_import_deletion(kind: str, batch_id: str, force: bool = False, user: dict = Depends(get_current_user)):
    require_admin(user)
    _, batch = await batch_document(get_db(), kind, batch_id)
    changes, warnings, blocked = await deletion_plan(get_db(), kind, batch, force=force)
    return ImportDeletionPreview(filename=batch["filename"], status=batch.get("status") or "Completed",
        remove=sum(c["before"] is None for c in changes), restore=sum(c["before"] is not None for c in changes),
        warnings=warnings, blocked=blocked)


@phed_router.delete("/import-batches/{kind}/{batch_id}", response_model=ImportDeleteResult)
async def delete_import_batch(kind: str, batch_id: str, data: ImportDeleteIn, user: dict = Depends(get_current_user)):
    require_admin(user)
    try:
        result = await undo_batch(get_db(), kind, batch_id, data.confirmation, user, force=data.force)
        await audit(user, "IMPORT_DELETE", "import", batch_id, meta={"kind": kind, "force": data.force, **result})
        return ImportDeleteResult(**result)
    finally:
        await clear_map_cache()
...
async def search_consumers(
    q: str = Query(..., min_length=1), limit: int = 20,
    document_status: Optional[OfficeDocumentFilter] = None, user: dict = Depends(get_current_user),
):
    db = get_db()
    receipt_filter = office_document_query(document_status)
    qn, qt = norm_id(q), norm_text(q)
    exact_ids = set()
    exact = await db.phed_consumers.find({**receipt_filter, "$or": [{"consumer_id_norm": qn}, {"phone_norm": qn}, {"provisional_ref": qn}]}, {"_id": 0}).limit(limit).to_list(None)
    ...
    if refs:
        more = await db.phed_consumers.find({**receipt_filter, "id": {"$in": refs}}, {"_id": 0}).to_list(None)
        exact.extend(more)
    ...
    if prop_hits:
        more = await db.phed_consumers.find({**receipt_filter, "linked_property_id": {"$in": [p["id"] for p in prop_hits]}, "id": {"$nin": list(exact_ids)}}, {"_id": 0}).to_list(None)
        exact.extend(more)
        exact_ids.update(c["id"] for c in more)
    partial = []
    if len(exact) < limit and len(qt) >= 2:
        ...
        name_prefix = await db.phed_consumers.find(
            {**receipt_filter, "id": {"$nin": list(skip_ids)}, "$or": [{"name_norm": prefix_rx}, {"fh_name": prefix_rx}]},
        ...
        if len(partial) < remaining:
            ...
            more = await db.phed_consumers.find(
                {**receipt_filter, "id": {"$nin": list(skip_ids)}, "$or": [{"name_norm": rx}, {"fh_name": rx}, {"address_norm": rx}, {"locality": rx}, {"consumer_id_norm": {"$regex": re.escape(qn), "$options": "i"}}, {"phone_norm": {"$regex": re.escape(qn)}}]},
...
async def survey_consumers(
    ...
    document_status: Optional[OfficeDocumentFilter] = None,
    page: int = 1,
    limit: int = 40,
    user: dict = Depends(get_current_user),
):
    ...
    db = get_db()
    base: Dict[str, Any] = {"is_active": True, "source": {"$ne": "survey"}, **office_document_query(document_status)}
    clauses: List[dict] = []
    ...
    if property_ids:
        ...
        from survey_state import enrich_survey_states
        await enrich_survey_states(db, properties)
    ...
    for item in items:
        ...
        item["survey_status"] = (item["linked_property"] or {}).get("phed_survey_state", "Not Started")
    return {
...
async def list_consumers(
    ...
    source: Optional[str] = None, document_status: Optional[OfficeDocumentFilter] = None,
    user: dict = Depends(get_current_user),
):
    require_officer(user)
    db = get_db()
    q: Dict[str, Any] = {"is_active": True, **office_document_query(document_status)}
...
async def update_property_location(property_record_id: str, data: LocationIn, user: dict = Depends(get_current_user)):
    ...
    if not is_officer(user) and prop.get("assigned_employee_id") != user["id"] and user["id"] not in (prop.get("assigned_employee_ids") or []):
        raise HTTPException(403, "Only assigned property pins can be moved")
...
class OfficeDocumentReceiptIn(BaseModel):
    property_record_id: str = Field(..., min_length=1, max_length=200)
    status: OfficeDocumentStatus
    remarks: str = Field("", max_length=2000)
    confirm: bool = False


class OfficeDocumentReceipt(BaseModel):
    property_record_id: str
    property_id: str
    status: OfficeDocumentStatus
    remarks: str
...
@phed_router.post("/consumers/{consumer_ref}/office-document-receipt")
async def save_office_document_receipt(
    consumer_ref: str, data: OfficeDocumentReceiptIn, user: dict = Depends(get_current_user),
):
    """Office document receipt: links the Property ID to the PHED consumer and submits the
    survey to the admin approval queue (so it can be approved / returned / rejected)."""
    if not data.confirm:
        raise HTTPException(400, "Confirm submission of the document receipt")
    remarks = data.remarks.strip()
    if data.status == "Other" and not remarks:
        raise HTTPException(400, "Please describe the documents received for Other")
    db = get_db()
    consumer = await db.phed_consumers.find_one({"id": consumer_ref, "is_active": True}, {"_id": 0})
    if not consumer:
        raise HTTPException(404, "Consumer not found")
    prop = await get_property_or_403(data.property_record_id, user)
    existing = await db.phed_surveys.find_one({"property_record_id": prop["id"], "status": {"$ne": "Rejected"}}, {"_id": 0}, sort=[("updated_at", -1)])
    if existing and existing["status"] == "Approved":
        raise HTTPException(409, "Survey already approved; ask an administrator to return it before changing the link")
    if existing and existing.get("surveyor_id") != user["id"] and not is_admin(user):
        raise HTTPException(403, "This survey belongs to another surveyor")
    ...
    ts = now_iso()
    receipt = OfficeDocumentReceipt(
        property_record_id=prop["id"], property_id=prop["property_id"], status=data.status,
        remarks=remarks, received_at=ts, received_by=user["id"],
        received_by_name=user.get("name") or user.get("username") or user["id"],
    )
    # 1) link the PHED consumer (and its connections) to the property
    link_upd: Dict[str, Any] = {
        "office_document_receipt": receipt.model_dump(), "linked_property_id": prop["id"],
        "linked_property_number": prop["property_id"], "status": "Surveyed", "updated_at": ts,
    }
    if not consumer.get("colony_name"):
        link_upd["colony_name"] = prop.get("colony")
    await db.phed_consumers.update_one({"id": consumer_ref}, {"$set": link_upd})
    await db.phed_connections.update_many({"consumer_ref": consumer_ref, "is_active": True},
                                          {"$set": {"linked_property_id": prop["id"], "updated_at": ts}})

    # 2) create / update the PHED survey and push it to the admin approval queue
    conns = await db.phed_connections.find({"consumer_ref": consumer_ref, "is_active": True}, {"_id": 0}).to_list(None)
    water_numbers = [c["connection_number"] for c in conns if c.get("service") == "Water" and c.get("connection_number")]
    sewer_numbers = [c["connection_number"] for c in conns if c.get("service") == "Sewer" and c.get("connection_number")]
    water = {
        "consumer_ref": consumer_ref,
        "consumer_id": consumer.get("consumer_id"), "consumer_name": consumer.get("consumer_name"),
        "mobile": consumer.get("phone") or prop.get("mobile"), "category": consumer.get("category"),
        "has_connection": bool(water_numbers or sewer_numbers),
        "connection_numbers": water_numbers, "sewer_connection_numbers": sewer_numbers,
        "has_water": bool(water_numbers), "has_sewer": bool(sewer_numbers),
        "document_pending": False, "missing_documents": [],
        "office_documents_submitted": True, "office_documents_recorded_at": ts,
        "office_documents_recorded_by": user["id"], "office_documents_recorded_by_name": receipt.received_by_name,
        "office_document_status": data.status, "office_document_remarks": remarks,
    }
    # The receipt label carries the field decision — reflect it on the survey.
    if data.status == "Sewer documents received at office":
        water["has_sewer"] = True
        water["has_connection"] = True
    elif data.status == "Death transfer documents received at office":
        water["owner_change"] = "DEATH_TRANSFER"
    elif data.status == "Owner change Documents received at office":
        water["owner_change"] = "OWNERSHIP_CHANGE"
    ward = await db.phed_wards.find_one({"colonies": prop.get("colony")}, {"_id": 0}) if prop.get("colony") else None
    survey_seed = {"survey_type": "EXISTING_LINKED", "water": water}
    code = _outcome_code(survey_seed, False)
    ref = (existing or {}).get("reference_number")
    if not ref or (existing or {}).get("reference_code") != code:
        ref = await _next_reference(db, code)
    fields = {
        "survey_type": "EXISTING_LINKED", "consumer_refs": list(dict.fromkeys([*((existing or {}).get("consumer_refs") or []), consumer_ref])), "consumer_ref": consumer_ref,
        "water": water, "status": "Submitted", "submitted_at": ts, "updated_at": ts,
        "reference_number": ref, "reference_code": code, "review_reasons": [],
        "source": "office_document", "rejection_reason": None, "return_reason": None,
        "ward_id": ward["id"] if ward else None,
        "ward_number": ward["ward_number"] if ward else prop.get("ward"),
        "colony_name": prop.get("colony"),
        "surveyor_id": user["id"], "surveyor_name": user.get("name") or user.get("username"),
        "remarks": remarks or (existing or {}).get("remarks"),
        "approved_at": None, "approved_by": None, "approved_by_name": None,
    }
    if existing:
        survey_id = existing["id"]
        await db.phed_surveys.update_one({"id": survey_id}, {"$set": fields})
    else:
        survey_id = str(uuid.uuid4())
        await db.phed_surveys.insert_one({
            "id": survey_id, "property_record_id": prop["id"], "property_id": prop["property_id"],
            "attachments": [], "started_at": ts, "created_at": ts, **fields,
        })
    await db.properties.update_one({"id": prop["id"]}, {"$set": {
        "phed_survey_status": "Submitted", "phed_survey_state": "Submitted", "phed_survey_type": "EXISTING_LINKED",
        "phed_surveyed_at": ts, "phed_outcome": _survey_outcome(fields),
        "phed_surveyor_id": user["id"], "phed_surveyor_name": fields["surveyor_name"],
    }})
    await clear_map_cache()
    await audit(user, "OFFICE_DOCUMENT_RECEIPT", "consumer", consumer_ref,
                before=consumer.get("office_document_receipt"), after=receipt.model_dump(),
                meta={"property_id": prop["property_id"], "survey_id": survey_id, "reference_number": ref})
    await audit(user, "SURVEY_SUBMIT", "survey", survey_id, after={"status": "Submitted", "reference_number": ref},
                meta={"property_id": prop["property_id"], "via": "office_document_receipt"})
    return {"receipt": receipt.model_dump(), "linked": True, "linked_property_id": prop["id"],
            "linked_property_number": prop["property_id"], "survey_id": survey_id,
            "survey_status": "Submitted", "reference_number": ref,
            "message": "Property ID linked and survey sent for admin approval"}
...
# ---------------- bulk unlink wrong consumer-property attachments ----------------
def _link_source(c: dict) -> str:
...
def _links_query(search: Optional[str], colony: Optional[str], link_source: Optional[str]) -> dict:
    q: Dict[str, Any] = {"is_active": True, "linked_property_id": {"$ne": None}}
    if colony and colony.strip():
        q["colony_name"] = {"$regex": f"^{re.escape(colony.strip())}$", "$options": "i"}
    if search and search.strip():
        rx = {"$regex": re.escape(search.strip()), "$options": "i"}
        q["$or"] = [{"consumer_id": rx}, {"consumer_name": rx}, {"phone": rx}, {"linked_property_number": rx}]
    if link_source == "office":
        q["office_document_receipt"] = {"$ne": None}
    elif link_source == "import":
        q["office_document_receipt"] = None
        q["import_id"] = {"$ne": None}
    elif link_source == "manual":
        q["office_document_receipt"] = None
        q["import_id"] = None
    return q


@phed_router.get("/links")
async def list_links(search: Optional[str] = None, colony: Optional[str] = None,
                     link_source: Optional[str] = None, page: int = 1, limit: int = 25,
                     user: dict = Depends(get_current_user)):
    """Admin: every consumer→property attachment, to find and remove wrong links."""
    require_admin(user)
    db = get_db()
    q = _links_query(search, colony, link_source)
    page = max(1, page)
    limit = min(max(1, limit), 200)
    total = await db.phed_consumers.count_documents(q)
    items = await db.phed_consumers.find(q, {"_id": 0, "original_values": 0}).sort("updated_at", -1).skip((page - 1) * limit).limit(limit).to_list(None)
    prop_ids = list({c["linked_property_id"] for c in items if c.get("linked_property_id")})
    props = {p["id"]: p for p in await db.properties.find(
        {"id": {"$in": prop_ids}},
        {"_id": 0, "id": 1, "property_id": 1, "owner_name": 1, "colony": 1, "phed_survey_status": 1}).to_list(None)} if prop_ids else {}
    office_prop_ids = [c["linked_property_id"] for c in items if c.get("office_document_receipt") and c.get("linked_property_id")]
    office_survey_props = set()
    if office_prop_ids:
        async for s in db.phed_surveys.find(
                {"property_record_id": {"$in": office_prop_ids}, "survey_type": "EXISTING_LINKED",
                 "source": "office_document", "status": {"$ne": "Approved"}}, {"_id": 0, "property_record_id": 1}):
            office_survey_props.add(s["property_record_id"])
    links = []
    for c in items:
        p = props.get(c.get("linked_property_id")) or {}
        links.append({
            "consumer_ref": c["id"], "consumer_id": c.get("consumer_id"), "consumer_name": c.get("consumer_name"),
            "phone": c.get("phone"), "colony_name": c.get("colony_name"), "ward_number": c.get("ward_number"),
            "consumer_status": c.get("status"),
            "linked_property_id": c.get("linked_property_id"), "linked_property_number": c.get("linked_property_number"),
            "property_owner": p.get("owner_name"), "property_colony": p.get("colony"),
            "property_survey_status": p.get("phed_survey_status"),
            "link_source": _link_source(c),
            "has_office_survey": c.get("linked_property_id") in office_survey_props,
        })
    return {"total": total, "page": page, "pages": (total + limit - 1) // limit if total else 0, "links": links}
...
class BulkUnlinkIn(BaseModel):
    ...
    confirm: str = ""
    remove_office_surveys: bool = True
    delete_consumers: bool = False


async def _unblock_property_after_unlink(db, prop_id: str):
    """Restore a property's survey state from its latest remaining survey; if none is left,
    reset it to untouched so surveyors can start field work again."""
    ...
    if remaining:
        await db.properties.update_one({"id": prop_id}, {"$set": {
            ...
            "phed_outcome": _survey_outcome(remaining) if remaining.get("status") == "Approved" else None,
            "phed_surveyor_id": remaining.get("surveyor_id"), "phed_surveyor_name": remaining.get("surveyor_name"),
        }})
    else:
        await db.properties.update_one({"id": prop_id}, {"$unset": {
            "phed_survey_status": "", "phed_survey_state": "", "phed_survey_type": "",
            "phed_surveyed_at": "", "phed_outcome": "", "phed_surveyor_id": "", "phed_surveyor_name": ""}})


@phed_router.post("/links/bulk-unlink")
async def bulk_unlink(data: BulkUnlinkIn, user: dict = Depends(get_current_user)):
    """Admin: detach wrong consumer→property links in bulk. Optionally removes the auto-created
    office-document surveys blocking those properties, and/or deletes the consumer records."""
    require_admin(user)
    if data.confirm != "UNLINK":
        raise HTTPException(400, "Type UNLINK to confirm")
    db = get_db()
    proj = {"_id": 0, "id": 1, "linked_property_id": 1, "linked_property_number": 1, "office_document_receipt": 1}
    if data.all_matching:
        consumers = await db.phed_consumers.find(_links_query(None, data.colony, data.link_source), proj).to_list(None)
    else:
        refs = [r for r in (data.consumer_refs or []) if r]
        if not refs:
            raise HTTPException(400, "Select at least one link, or use all_matching")
        consumers = await db.phed_consumers.find({"id": {"$in": refs}, "linked_property_id": {"$ne": None}}, proj).to_list(None)
    if not consumers:
        return {"message": "No linked consumers found", "unlinked": 0, "office_surveys_removed": 0,
                "properties_unblocked": 0, "deleted_consumers": 0, "deleted_connections": 0}
    ts = now_iso()
    affected_props = set()
    for c in consumers:
        prop_id = c.get("linked_property_id")
        upd = {"linked_property_id": None, "linked_property_number": None, "updated_at": ts}
        if data.remove_office_surveys and c.get("office_document_receipt"):
            upd["office_document_receipt"] = None
            upd["status"] = "Not Started"
        await db.phed_consumers.update_one({"id": c["id"]}, {"$set": upd})
        await db.phed_connections.update_many({"consumer_ref": c["id"]}, {"$set": {"linked_property_id": None, "updated_at": ts}})
        if prop_id:
            affected_props.add(prop_id)
    surveys_deleted = 0
    if data.remove_office_surveys and affected_props:
        res = await db.phed_surveys.delete_many({
            "property_record_id": {"$in": list(affected_props)}, "survey_type": "EXISTING_LINKED",
            "source": "office_document", "status": {"$ne": "Approved"}})
        surveys_deleted = res.deleted_count
    for prop_id in affected_props:
        await _unblock_property_after_unlink(db, prop_id)
    deleted_consumers = deleted_connections = 0
    if data.delete_consumers:
        refs = [c["id"] for c in consumers]
        deleted_connections = (await db.phed_connections.delete_many({"consumer_ref": {"$in": refs}})).deleted_count
        deleted_consumers = (await db.phed_consumers.delete_many({"id": {"$in": refs}})).deleted_count
    await clear_map_cache()
    await audit(user, "CONSUMER_BULK_UNLINK", "consumer", "bulk", meta={
        "unlinked": len(consumers), "office_surveys_removed": surveys_deleted,
        "properties_unblocked": len(affected_props), "consumers_deleted": deleted_consumers,
        "connections_deleted": deleted_connections, "all_matching": data.all_matching,
        "colony": data.colony, "link_source": data.link_source})
    return {"message": "Links removed", "unlinked": len(consumers), "office_surveys_removed": surveys_deleted,
            "properties_unblocked": len(affected_props), "deleted_consumers": deleted_consumers,
...
async def property_phed_section(property_record_id: str, user: dict = Depends(get_current_user)):
    ...
    survey = await db.phed_surveys.find_one({"property_record_id": property_record_id}, {"_id": 0}, sort=[("updated_at", -1)])
...
class SurveyDraftIn(BaseModel):
    ...
    office_documents_submitted: Optional[StrictBool] = None


def survey_public(s: dict) -> dict:
    s = dict(s)
    s['connection_type'] = connection_code(s)
    s['connection_label'] = CONNECTION_LABELS[s['connection_type']]
...
async def save_draft(data: SurveyDraftIn, user: dict = Depends(get_current_user)):
    ...
    if poly and centroid:
        ...
        await clear_map_cache()
    ward = await db.phed_wards.find_one({"colonies": prop.get("colony")}, {"_id": 0}) if prop.get("colony") else None
    previous_water = (existing or {}).get("water") or {}
    water = office_declaration_water(
        data.water if data.water is not None else previous_water, previous_water,
        data.office_documents_submitted, data.survey_type, user, ts,
    )
    fields = {
    ...
    if existing:
        result = await db.phed_surveys.update_one({"id": existing["id"], "status": {"$ne": "Approved"}, "updated_at": existing.get("updated_at")}, {"$set": fields})
        if not result.matched_count:
            raise HTTPException(409, "Survey changed; reopen it before saving")
        await clear_map_cache()
        if bool(previous_water.get("office_documents_submitted")) != water["office_documents_submitted"]:
            await audit(user, "SURVEY_OFFICE_DOCUMENTS", "survey", existing["id"],
                        after={"office_documents_submitted": water["office_documents_submitted"]})
    ...
    doc.pop("_id", None)
    if water["office_documents_submitted"]:
        await audit(user, "SURVEY_OFFICE_DOCUMENTS", "survey", doc["id"],
                    after={"office_documents_submitted": True, "recorded_at": ts})
    # PHED status lives in its own field; the legacy property-survey status is untouched.
    await db.properties.update_one({"id": prop["id"]}, {"$set": {"phed_survey_status": "Draft", "phed_survey_state": "Draft"}})
    await clear_map_cache()
...
class SurveySubmitResponse(BaseModel):
    ...
    reference_code: str
    office_documents_submitted: bool
    submitted_at: str
    connection_type: str


@phed_router.post("/surveys/{survey_id}/submit", response_model=SurveySubmitResponse)
async def submit_survey(survey_id: str, user: dict = Depends(get_current_user)):
    ...
    validate_office_submission(survey)
    ...
    if survey["survey_type"] == "WATER_CONNECTION":
        ...
        existing_c = None
        if w.get("consumer_ref"):
            existing_c = await db.phed_consumers.find_one({"id": w["consumer_ref"], "is_active": True}, {"_id": 0})
            if not existing_c:
                raise HTTPException(409, "Selected consumer no longer exists; select it again")
        elif explicit_id(w.get("consumer_id")):
            matches = await db.phed_consumers.find({"consumer_id_norm": norm_id(w["consumer_id"]), "is_active": True}, {"_id": 0}).limit(2).to_list(None)
            if len(matches) > 1:
                raise HTTPException(409, "Consumer ID has duplicate records; administrator review required")
            existing_c = matches[0] if matches else None
        else:
        ...
        if existing_c and existing_c.get("linked_property_id") not in (None, survey["property_record_id"]):
            raise HTTPException(409, "Consumer is linked to another property; administrator review required")
        cdoc = {
            "consumer_name": (w.get("consumer_name") or (prop or {}).get("owner_name") or "").strip(),
            "fh_name": (w.get("fh_name") or "").strip(), "phone": str(w.get("mobile") or w.get("phone") or (prop or {}).get("mobile") or "").strip(),
        ...
        if existing_c:
            cid = existing_c["id"]
            fields = cdoc if existing_c.get("source") == "survey" else {k: cdoc[k] for k in ("linked_property_id", "linked_property_number", "status", "updated_at")}
            await db.phed_consumers.update_one({"id": cid}, {"$set": fields})
        else:
            cid = str(uuid.uuid4())
        ...
        connection_pairs = [("Water", number) for number in conn_numbers] + [("Sewer", str(number).strip()) for number in (w.get("sewer_connection_numbers") or []) if str(number).strip()]
        for service, cn in connection_pairs:
            if not await db.phed_connections.find_one({"consumer_ref": cid, "connection_number_norm": norm_id(cn), "service": service, "is_active": True}):
                await db.phed_connections.insert_one({"id": str(uuid.uuid4()), "consumer_ref": cid, "consumer_id": cdoc["consumer_id"] or f"PROV-{survey['property_id']}",
                                                      "connection_number": cn, "connection_number_norm": norm_id(cn), "service": service, "category": cdoc["category"], "category_raw": cdoc["category"],
    ...
    return SurveySubmitResponse(message="PHED survey submitted", status=new_status, survey_id=survey_id,
                                reference_number=ref, reference_code=code,
                                office_documents_submitted=bool(w_meta.get("office_documents_submitted")),
                                submitted_at=ts, connection_type=connection_code(survey))
...
async def office_import(file: UploadFile = File(...), user: dict = Depends(get_current_user)):
    ...
    await db.phed_office_imports.insert_one({"id": batch_id, "filename": file.filename,
        "uploaded_by": user["id"], "uploaded_by_name": user.get("name"), "status": "Processing",
        "rollback_version": 1, "created_at": ts})
    for idx, r in enumerate(rows[1:], start=2):
        if not r or all(v in (None, "") for v in r):
            continue
        res["total_rows"] += 1
        pid, cid = explicit_id(get(r, "property_id")), explicit_id(get(r, "consumer_id"))
        if not pid and not cid:
            res["invalid"] += 1; unmatched.append({"row": idx, "reason": "No Property ID or Consumer ID"}); continue
        prop = None
        consumer = None
        if pid:
            matches = await db.properties.find({"property_id": {"$regex": f"^{re.escape(pid)}$", "$options": "i"}}, {"_id": 0}).limit(2).to_list(None)
            if len(matches) != 1:
                res["not_found"] += 1
                unmatched.append({"row": idx, "property_id": pid, "consumer_id": cid, "reason": "Property ID not found or ambiguous; no name/mobile fallback"})
                continue
            prop = matches[0]
        if cid:
            matches = await db.phed_consumers.find({"consumer_id_norm": norm_id(cid), "is_active": True}, {"_id": 0}).limit(2).to_list(None)
            if len(matches) != 1:
                res["not_found"] += 1
                unmatched.append({"row": idx, "property_id": pid, "consumer_id": cid, "reason": "Consumer ID not found or ambiguous"})
                continue
            consumer = matches[0]
            if prop and consumer.get("linked_property_id") not in (None, prop["id"]):
                res["invalid"] += 1
                unmatched.append({"row": idx, "property_id": pid, "consumer_id": cid, "reason": "Consumer ID is already linked to a different Property ID"})
                continue
            if not prop and consumer.get("linked_property_id"):
                prop = await db.properties.find_one({"id": consumer["linked_property_id"]}, {"_id": 0})
        if not prop:
            res["not_found"] += 1
            unmatched.append({"row": idx, "property_id": pid, "consumer_id": cid, "reason": "An explicit Property ID or an existing verified consumer-property link is required; names/mobile are not used"})
            continue
        existing = await db.phed_surveys.find_one({"property_record_id": prop["id"], "status": {"$ne": "Rejected"}}, {"_id": 0})
        if existing and existing["status"] == "Approved":
        ...
        if existing:
            await import_write(db, batch_id, "phed_surveys", existing["id"], upd)
            sid = existing["id"]
        else:
            sid = str(uuid.uuid4())
            ref = await _next_reference(db, "OF")
            await import_write(db, batch_id, "phed_surveys", sid, {"id": sid, "property_record_id": prop["id"], "property_id": prop["property_id"], "surveyor_id": user["id"], "surveyor_name": user.get("name"),
        ...
        await import_write(db, batch_id, "properties", prop["id"], {"phed_survey_status": "Approved", "phed_survey_type": "WATER_CONNECTION", "phed_surveyed_at": ts,
                                                                     "phed_survey_state": "Approved", "phed_outcome": "HAS_CONNECTION", "phed_survey_source": "office"})
        if consumer:
            await import_write(db, batch_id, "phed_consumers", consumer["id"], {"status": "Surveyed", "linked_property_id": prop["id"], "linked_property_number": prop["property_id"], "updated_at": ts})
            connections = await db.phed_connections.find({"consumer_ref": consumer["id"], "is_active": True}, {"_id": 0}).to_list(None)
            for connection in connections:
                await import_write(db, batch_id, "phed_connections", connection["id"], {"linked_property_id": prop["id"], "updated_at": ts})
        res["approved"] += 1
    await clear_map_cache()
    await audit(user, "OFFICE_IMPORT", "import", batch_id, meta={**res, "filename": file.filename})
    await db.phed_office_imports.update_one({"id": batch_id}, {"$set": {"status": "Completed", "result": res, "unmatched": unmatched[:500]}})
...
async def my_surveys(status: Optional[str] = None, page: int = Query(1, ge=1), limit: int = Query(30, ge=1, le=100), user: dict = Depends(get_current_user)):
...
async def list_surveys(
    ...
    if date_from or date_to:
        q["submitted_at"] = indian_date_range(date_from, date_to)
    if connection_type:
        q.setdefault("$and", []).append(connection_query(connection_type))
    if search and search.strip():
        rx = {"$regex": re.escape(search.strip()), "$options": "i"}
...
async def reopen_survey(survey_id: str, reason: str = Form(""), mode: str = Form("correction"), user: dict = Depends(get_current_user)):
    ...
    # Returned surveys require fresh confirmation; never silently reuse an office waiver.
    water["office_documents_submitted"] = False
    if note:
...
async def my_phed_progress(user: dict = Depends(get_current_user)):
    ...
    assigned_ids = await db.properties.distinct('id', assigned_q)
    survey_rows = await db.phed_surveys.find(
        {'property_record_id': {'$in': assigned_ids}, 'status': {'$in': completed_states}},
        {'_id': 0, 'water': 1, 'survey_type': 1, 'property_record_id': 1},
    ).sort('updated_at', 1).to_list(None)
    latest = {s['property_record_id']: s for s in survey_rows}
    category_counts = {code: 0 for code in CONNECTION_LABELS}
    for s in latest.values(): category_counts[connection_code(s)] += 1
    field_properties = await db.properties.count_documents({"source": "field", "created_by": uid})

    # Surveyor's OWN submitted PHED surveys (attributed by their ID): today + all-time
    # totals and per-connection-type breakdown, so the field dashboard matches the
    # admin "today" report the same way (per-surveyor, by connection_code).
    own_surveys = await db.phed_surveys.find(
        {"surveyor_id": uid, "status": {"$in": completed_states}},
        {"_id": 0, "submitted_at": 1, "water": 1, "survey_type": 1},
    ).to_list(None)
    today_ist = datetime.now(IST).strftime("%Y-%m-%d")
    my_conn = {code: 0 for code in CONNECTION_LABELS}
    my_today = 0
    for s in own_surveys:
        my_conn[connection_code(s)] += 1
        sa = s.get("submitted_at")
        if sa:
            try:
                dtp = datetime.fromisoformat(str(sa).replace("Z", "+00:00"))
                if dtp.tzinfo is None:
                    dtp = dtp.replace(tzinfo=timezone.utc)
                if dtp.astimezone(IST).strftime("%Y-%m-%d") == today_ist:
                    my_today += 1
    ...
    return {
        ...
        "already_connection": category_counts['ALREADY_CONNECTION'],
        "water_connection": category_counts['WATER_CONNECTION'],
        "sewer_connection": category_counts['SEWER_CONNECTION'],
        "new_connection": category_counts['NEW_CONNECTION'],
        "ownership_change": category_counts['OWNERSHIP_CHANGE'],
        "death_transfer": category_counts['DEATH_TRANSFER'],
        "field_properties": field_properties,
        "my_today_submitted": my_today,
        "my_total_submitted": len(own_surveys),
        "my_already_connection": my_conn['ALREADY_CONNECTION'],
        "my_water_connection": my_conn['WATER_CONNECTION'],
        "my_sewer_connection": my_conn['SEWER_CONNECTION'],
        "my_new_connection": my_conn['NEW_CONNECTION'],
        "my_ownership_change": my_conn['OWNERSHIP_CHANGE'],
        "my_death_transfer": my_conn['DEATH_TRANSFER'],
    }
...
async def phed_dashboard(ward_id: Optional[str] = None, colony: Optional[str] = None, surveyor_id: Optional[str] = None, status: Optional[str] = None,
    ...
    outcome_base = {**sq, 'status': {'$in': ['Submitted', 'Requires Review', 'Document Pending', 'Approved']}}
    counts = await asyncio.gather(*[
        db.phed_surveys.count_documents({'$and': [outcome_base, connection_query(code)]})
        for code in CONNECTION_LABELS
    ])
    by_outcome = dict(zip(CONNECTION_LABELS, counts))
    ...
    today_iso = indian_date_range(datetime.now(IST).date().isoformat())['$gte']
    ...
    return {
        ...
        "by_outcome": {**{code.lower(): count for code, count in by_outcome.items()}, 'has_connection': by_outcome['ALREADY_CONNECTION']},
...
def _survey_outcome_code(survey: dict) -> str:
    code = connection_code(survey)
    return {'ALREADY_CONNECTION': 'already_verified', 'UNKNOWN': 'other'}.get(code, code.lower())
...
async def phed_today_dashboard(
    ...
    """PHED survey progress and surveyor-wise operational report (today by default, all-time when scope='all')."""
    require_officer(user)
    db = get_db()
    all_time = (scope or "").lower() == "all"
    today = datetime.now(IST).date().isoformat()
    today_range = indian_date_range(today, today)
    today_start = today_range['$gte']
    custom_range = indian_date_range(date_from, date_to) if (date_from or date_to) else None
    base: Dict[str, Any] = {}
    if ward_id:
    ...
    if not all_time:
        submitted_q["submitted_at"] = today_range
    if custom_range:
        submitted_q["submitted_at"] = custom_range
    projection = {
        "_id": 0,
        "id": 1,
    ...
    in_progress_q = {**base, "status": "Draft"}
    if not all_time:
        in_progress_q["updated_at"] = today_range
    if custom_range:
        in_progress_q["updated_at"] = custom_range
    ...
    locked_q: Dict[str, Any] = {"is_active": True, "linked_property_id": {"$ne": None}}
    if ward_id:
        locked_q["ward_id"] = ward_id
    if colony:
        locked_q["colony_name"] = colony
    locked_consumers = await db.phed_consumers.count_documents(locked_q)
    locked_imported = await db.phed_consumers.count_documents({**locked_q, "source": "imported"})
    ...
    totals = {
        ...
        "water_connection": 0,
        "sewer_connection": 0,
        "already_connection": 0,
        "death_transfer": 0,
        "ownership_change": 0,
        "locked_consumers": locked_consumers,
        "locked_consumers_imported": locked_imported,
        "properties_linked": len(linked_ids),
        "in_progress": len(in_progress),
    }
    by_surveyor: Dict[str, dict] = {}

    # connection_code() bucket -> per-surveyor / totals counter key the dashboard reads
    CONN_KEY = {
    ...
    for survey in surveys:
        ...
        conn_key = CONN_KEY.get(connection_code(survey))
        if conn_key:
            totals[conn_key] = totals.get(conn_key, 0) + 1
            summary[conn_key] = summary.get(conn_key, 0) + 1
        if survey.get("survey_type") == "NO_CONNECTION":
            totals["no_phed_connection"] += 1
        if survey.get("returned_at"):
            key = "marked_pending" if survey.get("return_mode") == "pending" else "returned_for_correction"
            totals[key] += 1
    ...
    for survey in sorted(surveys, key=lambda item: item.get("submitted_at") or "", reverse=True)[:50]:
        recent_surveys.append({
            ...
            "outcome": _survey_outcome_code(survey),
            "connection_type": connection_code(survey),
            "submitted_at": survey.get("submitted_at"),
        })

    return {
        "report_date": None if all_time else today,
        "scope": "all" if all_time else "today",
        "filters": {"ward_id": ward_id, "colony": colony, "surveyor_id": surveyor_id, "date_from": date_from, "date_to": date_to},
        "summary": totals,
...
async def filter_options(user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "survey_connection_types": list(CONNECTION_LABELS.items()),
...
async def export_phed(ward_id: Optional[str] = None, colony: Optional[str] = None, surveyor_id: Optional[str] = None, status: Optional[str] = None,
                      ...
                      search: Optional[str] = None, linked: Optional[str] = None,
                      document_status: Optional[OfficeDocumentFilter] = None, user: dict = Depends(get_current_user)):
    require_officer(user)
    ...
    db = get_db()
    cq: Dict[str, Any] = {"is_active": True, **office_document_query(document_status)}
    if linked == "yes": cq["linked_property_id"] = {"$ne": None}
    if linked == "no": cq["linked_property_id"] = None
    if search:
        qn, qt = norm_id(search), norm_text(search)
        conn_refs = await db.phed_connections.distinct("consumer_ref", {"connection_number_norm": {"$regex": re.escape(qn)}})
        rx = {"$regex": re.escape(qt), "$options": "i"}
        cq["$or"] = [{"consumer_id_norm": {"$regex": re.escape(qn)}}, {"phone_norm": {"$regex": re.escape(qn)}}, {"name_norm": rx}, {"address_norm": rx}, {"fh_name": rx}, {"locality": rx}, {"id": {"$in": conn_refs}}, {"linked_property_number": rx}]
    if ward_id: cq["ward_id"] = ward_id
    if colony: cq["colony_name"] = colony
    if category: cq["category"] = category
    ...
    headers = ["Town", "Ward", "Colony", "Property ID", "Property Owner", "Property Address", "Property Latitude", "Property Longitude",
               ...
               "Survey Latitude", "Survey Longitude", "GPS Accuracy (m)", "Submitted Date / Time (IST)", "Document Availability", "Remarks", "Import Source",
               "Office Document Status", "Receipt Property ID"]
    ws.append(headers)
    ...
    for c in consumers:
        ...
        row = [town.get("town") or town_code, w.get("ward_number") or c.get("ward_number") or p.get("ward") or ward_master.ward_for_colony(c.get("colony_name") or p.get("colony")) or "", c.get("colony_name") or p.get("colony") or "",
               ...
               CONNECTION_LABELS[connection_code(s)] if s else "Not surveyed",
               ...
               ist_datetime_text(s.get("submitted_at")), docs or ("None" if s else ""), s.get("remarks") or "",
               f"{c.get('source')}: {c.get('source_file') or ''} row {c.get('source_row') or ''}".strip(),
               (c.get("office_document_receipt") or {}).get("status") or "Not submitted",
               (c.get("office_document_receipt") or {}).get("property_id", "")]
        ws.append(row)
        r = ws.max_row
