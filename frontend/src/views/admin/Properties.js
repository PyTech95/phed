...
export default function Properties() {
  ...
  const [wardList, setWardList] = useState([]); // [{ward_number, name, colonies}]
  const [assignWard, setAssignWard] = useState('');
  const wardColonies = assignWard ? (wardList.find((w) => String(w.ward_number) === assignWard)?.colonies || []) : allColonies;
  ...
  const openBlockColonyDialog = async (mode) => {
    ...
    try {
      const [res, wr] = await Promise.all([
        axios.get(`${API_URL}/admin/colonies`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`${API_URL}/phed/wards`, { headers: { Authorization: `Bearer ${token}` } }).catch(() => ({ data: { wards: [] } })),
      ]);
      setAllColonies(res.data.colonies || []);
      setWardList((wr.data.wards || []).slice().sort((a, b) => (parseInt(a.ward_number, 10) || 0) - (parseInt(b.ward_number, 10) || 0)));
    } catch (error) {
        ...
        <Dialog open={blockColonyDialog} onOpenChange={setBlockColonyDialog}>
          <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <Label className="text-sm font-semibold">Ward (colonies filter)</Label>
                <select className="w-full h-10 border rounded-lg px-3 mt-1 bg-white text-sm" value={assignWard} onChange={(e) => { setAssignWard(e.target.value); setSelectedColonies([]); }} data-testid="assign-ward-select">
                  <option value="">All wards — सभी colonies</option>
                  {wardList.map((w) => <option key={w.id || w.ward_number} value={String(w.ward_number)}>Ward {w.ward_number}{w.name ? ` — ${w.name}` : ''} ({(w.colonies || []).length} colonies)</option>)}
                </select>
              </div>
              <div>
                <div className="flex items-center justify-between">
                  <Label className="text-sm font-semibold">Select Colonies ({selectedColonies.length} selected)</Label>
                  {wardColonies.length > 0 && (
                    <button type="button" className="text-xs text-blue-600 underline" onClick={() => setSelectedColonies(selectedColonies.length === wardColonies.length ? [] : [...wardColonies])} data-testid="assign-select-all-colonies">
                      {selectedColonies.length === wardColonies.length ? 'Clear all' : 'Select all'}
                    </button>
                  )}
                </div>
                <div className="max-h-40 overflow-y-auto border rounded-lg p-2 mt-1 space-y-1">
                  {wardColonies.length === 0 ? (
                    <p className="text-sm text-slate-400 py-2 text-center">{allColonies.length === 0 ? 'Loading colonies...' : 'इस ward में कोई colony mapped नहीं'}</p>
                  ) : wardColonies.map(colony => (
